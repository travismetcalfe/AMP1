...
1000  if(lines.ne.lkur) then
         if(istdpr.gt.0) 
     *  write(istdpr,*) 'kurucz opacity: wrong line number in table.'
         if(istdpr.gt.0) 
     *  write(istdpr,*) 'lines expected, read: ',lkur,lines
      else
         if(istdpr.gt.0) write(istdpr,'(2a,2i3,i5)') 
     *     'table successfully read. nt,np(nt),',
     +                         'lines = ',nt,np(nt),lines
      end if
c
      return
 9000 format(2f5.2,5f7.3,3f9.5,f8.3)
 9030 write(istdpr,*)'error in opening ',infile
      stop
c
      end
