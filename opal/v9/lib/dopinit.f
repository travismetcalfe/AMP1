...
c
c     07/06/02: include 'external itohco' statement to initialize
c               coefficients (particulary necessary on SIG/IRIX 6.x)
c
c     Last modification:
c     07/06/02
c
...
c
      external itohco   ! load coeffients for electron conduction
c
...
c     read ival-array from file 
     ...
     *  'read ival.dat for extrapolation domain checking'
      ...
      do 3001 l=1,nzvai
       if(nzvai.eq.13)then
c        OPAL95
         ioptab = 11         ! OPAL95+Alexander 94
         nxiri=nxir
         nyiri=nyir
         nyisi=nyis          !=5:AlEX94<->OPAL95 fit at log(T)=3.90<->3.95
         if(iii.ne.nzva)then !=7:Kuru91<->OPAL95 fit at log(T)=4.00<->4.05    
              ioptab=1       ! OPAL95+Kuruz91
              nyisi=nyisi+2
         endif
         mdi=md
