	subroutine readi (lun,optabe,ntab,nval,nlin,nzva,rlg,
     +                    tlg,opa,ival)
...
c	   read 'tlg' + opacity-values
 ...
 1015   continue
     ...
     *    'readi: last table ',j-1,' lines read'
c       close input file lun
        close(lun)
c
	return
c
 9001   write(istdou,*) 'readi: error in open statement'
        if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,*) 
     *    'readi: error in open statement'
	stop 'in dreadi'
 9002	write(istdou,*) 'readi: error in read: line: ',j
     	if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,*) 
     *    'readi: error in read: line: ',j
	stop 'in dreadi'
c
	end
