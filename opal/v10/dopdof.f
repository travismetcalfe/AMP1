...
c   20/08/97
c
d     external myhandleri
d     external myhandleru
d     external myhandlero
d     external myhandlerd
c
      character*79 tabnam
c      
      parameter(nn=200)
...
c
      data iexp/0/
      data tabnam /'OPINTPATH_95E','OPINTPATH_02E'/
      data iorder /4/
      data imode  /-1/
c
c     setting up floating-point exception handler
c
d     ieeer = ieee_handler ('set', 'invalid', myhandleri)
d     if (ieeer .ne. 0) write(istdpr,*) 'ieee_handler i cant be set !!'
d     ieeer = ieee_handler ('set', 'division', myhandlerd)
d     if (ieeer .ne. 0) write(istdpr,*) 'ieee_handler d cant be set !!'
d     ieeer = ieee_handler ('set', 'overflow', myhandlero)
d     if (ieeer .ne. 0) write(istdpr,*) 'ieee_handler o cant be set !!'
d     ieeer = ieee_handler ('set', 'underflow', myhandleru)
d     if (ieeer .ne. 0) write(istdpr,*) 'ieee_handler u cant be set !!'
c
c
      itab=1
      if(istdpr.gt.0) 
     *  write(istdpr,'(A,$)') 
     .        ' Enter low-T tables, 1=OPAL95E, 2=OPAL02E : '
...
c           (no extrapolation checking: ier=0 always)
            call opintf(xval,zval,tlg,rlg,opalg(i,j),opr(i,j),opt(i,j),
...
c            if(iexp.gt.0)print 6000,'opdof: extrapolated points at ( ',
c     +                           'rlg=',xi(i),' tlg=',tlg,
c     +                           ') : ',iexp
c
...
c
 9991 write(istdpr,*)'opdof: input error'
      stop
      end
c
