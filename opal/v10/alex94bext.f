...
c-----------read 1st line 
            ...
            write(istdpr,*)'read ',ifname(k),line(1:37)
            write(istdpr,*)'and extrapolate table for log(R)=[-8.,-7.5]'
...
c--------------read tlg,rlg and opacity-values
 ...
 9999          format(i2,i3,f6.3,f5.1,
     +                8f8.3,/,10f8.3,/,3f8.3)
               if(j.ne.it)then
                     write(istdpr,*)' error in temperature index....'
                     stop
               endif
               ir(k,l) = ir(k,l)+2                   ! should be 19
               if(ir(k,l).ne.nval) then
                    write(istdpr,*)' error in log(R) index....'
                    stop
               endif
               do i=1,nval
...
c
 9001   write(istdpr,*)
     *    'alex94bext: error in opening ',ifname(k),' lun= ',lun
	stop
 9011   write(istdpr,*)
     *    'alex94bext: error in opening ',ofname,' lun= ',lun
	stop
 9002	write(istdpr,*)'alex94bext: error in reading: line: ',j
	stop
c
	end
