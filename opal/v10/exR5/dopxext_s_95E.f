...
c
c     07/06/02: for g77 compatibility: redefine data statement for xtab
c
c    Last modification:
c    16/04/03
c
...
c
      ...
      parameter(nxir=27,nyir=70,nyis=7)
      parameter(niwk=70000,nwk=60000)
      parameter(ndat=2538)
      parameter(lun=31,nout=6)
      parameter(md=27,mt=30)
      parameter(ncp=7)
c
...
c
      ...
      print '(a,/)','--------------------------------------'
c     repeat for nzva tables (normaly: nzva=13)
      do 4010 l=1,nzva
       write(istdpr,*)'processing table Z= ',l
c      repeat for nxir density points (normaly: nxir=19)
       do 4020 i=1,nxir
c       repeat for (nyir-nyis) temp. points (normaly: nyir=70)
        do 4030 j=nyis,nyir
...
d     l=1
d     itab=1
d     write(istdpr,*)'output test-table l=',l
d     open(8)
d     open(9)
d     open(90)
d     open(91)
...
c
      do 6001 ls=1,nzva
        write(istdpr,*)'processing table: Z= ',ls
        do 6010 js=1,mt
          do 6020 is=1,nxir
c
...
9030  write(istdpr,*)'dopinit: error in opening of OPINTPATH'
      write(istdpr,*)
     *  'dopinit: try to assign a logical name to OPINTPATH'
      write(istdpr,*)
     *  'dopinit: with setenv OPINTPATH <absolute_path_name>'
      write(istdpr,*)
     *  'dopinit: in a csh where <absolute_path_name> is the'
      write(istdpr,*)
     *  'dopinit: file containing the absolute pathnames of'
      write(istdpr,*)'dopinit: the input files optabe.bin pderivs.dat'
      write(istdpr,*)'dopinit: and ival.dat'
      stop
c
      end
c
