...
c
      ...
      parameter(nxir=27,nyir=70,nyis=5)     ! OPAL95, nyis=7 for KURU91
      parameter(ntal=19)                    ! Alexander95
      parameter(md=27,mt=30)                ! Kurucz91
...
c     parameter(nt=3348,ial=2*ndat+5)       !ial=3235 for OPAL95+ALEX94
      parameter(nt=4836)                    ! nt=3024 for OPAL95+ALEX94
...
c
     ...
     *  'read partial derivative values and triangle indices...'
      if(nti.gt.nt)then
        ...
        if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdou,*) 
...
c        OPAL95
         if(nti.gt.3348)then
           nxiri=nxir       ! extrapolated OPAL95 tables
           mdi=md           ! extrapolated Kurucz tables
         else
           nxiri=19         ! original OPAL95 tables
           mdi=19           ! original Kurucz tables
         endif
         nyiri=nyir
         ...
         read(32,'(70i3)')((ivalo(m,k,l),m=1,nyiri),k=1,ntabi)  !OPAL95
         if(nti.gt.3348)then
            read(32,'(70i3)') (ivalk(m,l),m=1,mt)               !Kuru91 extrapolated
         else
            do m=1,mt
              ivalk(m,l)=mdi                                    !original Kuru91
            enddo
         endif
       else
c        OPAL92
...
c
          if(iii.eq.nzva)then
            ...
            do 1225 js=1,mt                     ! load Kurucz tables
             do 1221 is=1,nxiri                 ! of size nd=27x30=810 ...OPAL95
