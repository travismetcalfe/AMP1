...
c              tlg=  3.91 3.94 3.97 4.0  4.05       I... interpolated point)
c
c
c     13/11/95: modified for OPAL95 tables
c
c     25/05/97: interpolated in log(Z)
c
c     07/06/02: for g77 compatibility: replace .eq. -> .eqv. if logicals
c               are compared
c
c     05/10/02: modified for OPAL02 (Frank PIPERS)
c
c     parameter for Livermore-tables
c
      parameter(ntab=7,nzva=13)
      parameter(nxir=19,nyir=70)
      parameter(lunl=31,ntlgc=100)
      parameter(lunivk=51)
c
c     parameter for Kurucz-tables
c
      parameter(it=19,iz=13,irlt=27)
...
c
      ...
      double precision zku(it),zkul(it),zli(iz),zlil(iz)
...
c
      character*50 tabnam, outtab, fnivalk
...
c     table dimension for s/r opintc{f} and opints
      common /tabdim/ nzvai,ntabi,nxiri,nyiri,nyis,nyifi,mdi,
     +                nti,iali
c
...
c
      data zsun /1.886d-2/
      data tabnam /'KURTABS'/
c     data outtab /'kuru91_02.bin'/
      data lun /21/
      data zli /1.0d-7, 1.0d-4 , 3.0d-4 , 1.0d-3 , 2.0d-3,
     +          4.0d-3, 1.0d-2 , 2.0d-2 , 3.0d-2 , 4.0d-2,
     +          6.0d-2, 8.0d-2 , 1.0d-1/
      data rlt /-8.0d0,-7.5d0,-7.0d0,-6.5d0,-6.0d0,-5.5d0,-5.0d0,
...
c
     ...
     +     form='formatted',status='old',err=9030)
      ...
      print *,optabe,' (input)'
      print *,outtab,' (output)'
      print *,fnivalk,' (output)'
      call rdi95(lunl,optabe,ntab,nxir,nyir,nzva,rlgl,tlgl,opal,ival)
c
c======================================================================
c
c     do univariate interpolation in log(Z)
...
c
        call uvip3p(nd,itab,zkul,opk(1,i),iz,zlil,
     +                           opl(1,i)         )
c
c       interpolate the density values
c
        call uvip3p(nd,itab,zkul,dek(1,i),iz,zlil,
     +                           del(1,i)         )
3001  continue
c
c======================================================================
...
c     setup fitting temperature table points
      ...
      tlf(3)=tl(jlo)
c
c     open(lunivk,name=fnivalk)
      open(lunivk,name=fnivalk,ACCESS='APPEND')  ! 'APPEND' is f77 extension !
      do 5001 l=1,iz
c
...
5020       continue
           ivalk(i,l)=27
           if(rlg(mp).lt.5.0d0)ivalk(i,l)=int(rlg(mp)*2.0d0)+17
...
5005     continue
c
c        output ival-array for Kurucz tables
         write(lunivk,'(70i3)')(ivalk(jm,l),jm=1,jlo)
5001  continue
      close(lunivk)
c
c======================================================================
...
c
      stop
c
9030  print *,'ku2l2g_95: error in opening ',tabnam
      stop
9040  print *,'ku2l2g_95: array-dim error of it'
      stop
9050  print *,'ku2l2g_95: wrong nr. of tablepoints',ip,lkur
      stop
c
      end
