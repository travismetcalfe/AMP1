      subroutine rde05(lun,optabe,ntab,nval,nlin,nzva,rlg,
     +                    tlg,opa,ival)
      implicit double precision (a-h,o-z)
      character*(*) optabe
c
c      purpose:
c            read the binary OPAL05 values
c       reference: 
...
c      nzva ...... max. nr. of different z-values [z=13 for OPAL05]
...
c
 ...
 3001 continue
     ...
     *    'rde05: last table ',j-1,' lines read'
c       close input file lun
        close(lun)
c       close of output file lun+1 in calling routine (exop05.f)
c
      return
c
 9001   write(istdou,*) 'rde05: error in opening ',ifname,'lun= ',lun
        if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,*) 
     *    'rde05: error in opening ',ifname,'lun= ',lun
      stop 'in rde05'
 9011   write(istdou,*) 'rde05: error in opening ',optabe,'lun= ',lun+1
        if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,*) 
     *    'rde05: error in opening ',optabe,'lun= ',lun+1
      stop 'in rde05'
 9002   write(istdou,*) 'rde05: error in read: line: ',j
        if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,*) 
     *    'rde05: error in read: line: ',j
      stop 'in rde05'
c
      end
