...
c
     ...
     .'opints: using ',iorder+1,' ztabs ',ztab(nzvab),' ->',ztab(nzvae)
        write(istdpr,'(  x,a,i1,a,f8.4,a,f8.4,/)') 
     .'opints: using ',iorder+1,' xtabs ',xtab(ntabb),' ->',xtab(ntabe)
      end if
...
c
     ...
     +                 ra(1,1,1,1,itab,l),pp,
     +                 zii,pdx,pdy,ier)
             if(ier.gt.0.and.istdpr.gt.0)
     +          write(istdpr,*) 'opints: ***Error*** in extrapolation'
             zzshep(ii)=zii
             pxshep(ii)=pdx
             pyshep(ii)=pdy
             xshep(ii) =xii
