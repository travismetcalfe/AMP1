...
c
      subroutine opintc(xval,zval,tlgv,rlgv,opalg,opx,opy,
     +                  oph,opz,iexp,ier)
...
c
c     24Frb06 : modified for using extrapolated OPAL * AF05 tables
c
c     28Feb06 : introduce variable econ that sets treshold at which
c               electron conduction is added to the final opacity value
c
c     16Aug10: Add flag for initialization of opacity tables in
c              common/c_opinit/. Used in this routine to re-initialize
c              itpv and lfi
c
c     Last modification:
c     16Aug10
...
c
c---  parameter when electron conduction is added to final opacity value
      parameter(econ=5.0d0)
c
...
c---- values from s/r opinit used as argument for s/r  masubi
      common /tabdim/ nzvai,ntabi,nxiri,nyiri,nyisi,nyifi,
     +                nti,iali
c
...
c
      SAVE                   ! 040203
c
c  Test for re-initialization of lfi and itpv
c
      if(i_opinit.eq.1) then
        lfi = .true.
        itpv = 1
        if(istdpr.gt.0) write(istdpr,
     *    '(/'' In opints, itpv and lfi reset to 1 and TRUE''/)')
        i_opinit = 0
      end if
c
      if(zval.lt.1.0d-7)then
         if(istdpr.gt.0) write(istdpr,'(/,x,a)') 
     *     '***WARNING*** opintc: setting Z=1.d-7'
         zval=1.0d-7
         zvall=-7.0d0
      else
         zvall=dlog10(zval)
...
c
      if(istdpr.gt.0) then
	write(istdpr,'(/,x,a,i1,a,f8.4,a,f8.4)') 
     .'opintc: using ',iorder+1,' ztabs ',ztab(nzvab),' ->',ztab(nzvae)
        write(istdpr,'(  x,a,i1,a,f8.4,a,f8.4,/)') 
     .'opintc: using ',iorder+1,' xtabs ',xtab(ntabb),' ->',xtab(ntabe)
      end if
      endif    ! selection of Z-tables
c
...
c          check if point lies within extrapolation domain
           ...
           if((jlo.ge.nyis).and.(jlo.le.nyiri))then
              if(xi.gt.rlg(ivalo(jlo,itab,l),itab,l))iexp=iexp+1 ! OPAL
           else
              if(xi.gt.1.0d0) iexp=iexp+1                        ! AF05
           endif
 4001   continue
c
...
d     ier=2
d     return
d9730 print *,'opintc: error in common-array ndat'
d     ier=2
d     return
d9740 print *,'opintc: error in common-array ntab'
d     ier=2
d     return
d9750 print *,'opintc: error in common-array nzva'
d     ier=2
d     return
d9760 print *,'opintc: error in common-array niwkl'
d     ier=2
d     return
d9770 print *,'opintc: error in common-array nwkl'
d     ier=2
d     return
 9810 iexp=1
      if(istdpr.gt.0) then
        write(istdpr,*) 
     *    'opintc: **Warning** interpolation point outside',
     +'triangulation domain, apply extrapolation (Shepard''s method)'
         write(istdpr,'(a,4f9.5,/)') ' log(T), log(R), X, Z:',
     &      yi,xi,xval,zval
      end if
      ier=1
      ...
      do 5010 l=nzvab,nzvae
        do 5001 itab=ntabb,ntabe
          do 5020 ii=1,nexp+1
             ...
             if(ier.ne.0.and.istdpr.gt.0)
     +          write(istdpr,*) 'opintc: ***Error*** in extrapolation'
