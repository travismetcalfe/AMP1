...
c
c    computes partial derivatives for OPAL and Alexander-Furgenson (2005) 
c    as used for the minimum norm interpolation scheme
c
c    History:
c    19/08/97: creation from dopxext_s_95.f
c    05/10/02: modied for OPAL02 (Frank Pipers)
c    23/02/06: modied for Asplund, Grevesse and Sauval mixture (AGS05)
c
c    Last modification:
c    23/02/06
c
      ...
      parameter(niwk=700000,nwk=600000)
      parameter(lun=31,nout=6)
c
c
      character*75 optabe,paderi,ivadat,lowtab
c
c     for s/r masube
...
c
c     read the opacity-tables (OPAL + AF05)
      print *,'read OPAL tables...'
      call rdi95(lun,optabe,ntab,nxir,nyir,nzva,rlg,tlg,opa,ival)
      print *,'read AF05 tables...'
      lun2=lun+2
      call rdaf05(lun2,lowtab,ntab,nxir,ntal,nzva,rlgal,tlgal,opaal)
c
...
c
      print '(/,a)','evaluate partial derivatives for OPAL+AF05'
      print '(a,/)','----------------------------------------------'
      write(nout,8610)
c     repeat for nzva tables (nzva=13 for OPAL)
      do 8010 l=1,nzva
c
...
c       ( nxi x nyi coordinates at the desired z-values)
        do 8001 itab=1,ntab
          nd = 0
c         print *,'OPAL tlg=',tlg(nyis,itab,l)
c         print *,'AF05 tlg=',tlgal(1,itab,l)
          do 1002 j=nyis,nyir
           do 1001 i=1,nxir
               nd = nd + 1
...
9030  print *,'opafpd: ERROR in opening of OPINTPATH'
      stop
c
      end
c
