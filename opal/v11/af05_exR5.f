...
c
c     extrapolate Alexander-Fergunson (2005 format) tables to log(R)=5
c     using linear extrapolation (Akima: uvip3p.f)
c
c     hg: 10/04/2003
c     hg: 23/02/2006
c
      parameter(nx=8, nz=13)
      parameter(nr=19,nt=73)
      parameter(mr=27)
      parameter(lun=31,nd=5,n8=8,nr1=nr+1)
      character*70 optabe,paderi,ivadat,optabee
c
      dimension rlg95(nr),rlg95e(mr),rlge(n8),tlg95(nt)
c
c     dimensions for Alexander05 tables
      dimension rlg(nr,nx,nz)
      dimension tlg(nt,nx,nz)
      dimension opa(nr,nt,nx,nz)
      dimension opae(mr,nt,nx,nz)
      dimension iva(nr,nt,nx,nz)            ! needed for compatibility
c
...
c
c     setup the opal T values
      do 3510 i=1,nt
       tlg95(i)=tlg(i,nx,nz)
3510  continue
c
c     setup extended opa R values
      do 3515 i=1,nr
       rlg95(i) =rlg(i,nx,nz)
       rlg95e(i)=rlg95(i)
3515  continue
...
c     do the extrapolation
      do iz=1,nz
        do ix=1,nx
          do it=1,nt
c            if((tlg95(it).lt.3.11d0).or.(tlg95(it).gt.3.26))then
             if(tlg95(it).gt.3.30)then
                call uvip3p(nd,nr,rlg95,opa (  1,it,ix,iz),
     +                         n8,rlge, opae(nr1,it,ix,iz))
             else         ! now way to extrapolate reasonably for logT<3.30
                do ii=nr1,mr
                   opae(ii,it,ix,iz)=opa(nr,it,ix,iz) 
                enddo
             endif
             do ir=1,nr
...
c
c
c     output all extrapolated OPAL tables (format as in exakop05.f)
      ...
      close(lun+2)
c
c     output one extrapolated OPAL table
      ix=6        ! X=0.7
      iz=8        ! Z=0.02
