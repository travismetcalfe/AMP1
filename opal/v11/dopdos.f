...
c
      ...
      data iorder /4/
      data imode  /-2/     ! rat. splines (no electron conduction)
c
...
c
 ...
 321  write(6,'(A,$)')' Enter X-Value [0.-.8 ]: '
      ...
      if(xval.lt.0.d0.or.xval.gt.9d-1)goto 321
 322  write(6,'(A,$)')' Enter Z-Value [0.-.1]: '
      read(5,'(d12.5)',err=9991)zval
      if(zval.lt.0.d0.or.zval.gt.1d-1)goto 322
...
c
     ...
     +                  'opdos: error in interpolation s/r,ier= ',ier
c           check if point lies in extrapolation domain
            if(iexp.gt.0)print 6000,'opdos: extrapolated points at ( ',
     +                           'rlg=',xi(i),' tlg=',tlg,
     +                           ') : ',iexp
c
