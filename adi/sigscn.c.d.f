...
c  nfmode.
c
c  Modified 29/7/10, changing criterion for onset of fine scan. This
c  deserves refinement, better control.
c
...
c  Limit for test changed from 0.6 to 0.7 29/7/10, somewhat 
c  arbitrarily
c
        if(isig.gt.isigfn+1.and.detpp*det.gt.0.and.ifine.eq.0
     *    .and.2*detp/(detpp+det).le.0.7) then
c
          isigst=isig
	  sigfst=sig
	  sigpst=sigp
