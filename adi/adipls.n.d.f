...
c
      ...
      character tail*5, cntrd*30, ctrdsf*1
...
c
     ...
     *   idsrkr,idsgkr
c
      external rhs,rhsnrm,bcnrk,blstio
c
...
c
      data imldsp, itrdsp, idsgsp, idsssp,  idsmdp, idsrkp, 
     *     idsgkp, istdpp
     *     /  -1,     -1,     -1,     -1,     -1,     -1,
     *        -1,     -1  /
c
...
c
c  on d/s idsrkr the rotational kernel is written for irotkr = 1. 
...
c
      call zero(csummm,50)
c
c  hard-code output file unit numbers
c
      idsgsm=11
      idsssm=15
      idsefn=4
...
c  Note that istsig has special meaning when iscan .gt. 1;
c  see below.)
c
      itrsig=0
      sig1=10
...
c  frequency. When istsig = 2, sig1 and sig2 are taken as limits in
...
c  mdintg = 5: use fourth-order scheme, integrate from centre and
c              surface and get frequency from matching
      mdintg=1
...
c         the eigenfrequency is known.
      itmax=8
c  dsigre: when using Richardson extrapolation, decrease trial
c  sig by dsigre before iteration on thinned-out mesh
      dsigre=0
...
c          variational calculation at npvarf points.
      npvarf=0
c  nfmode: for nfmode .ge. 1 write eigenfunctions to file on d/s 4.
c  The variables output and the format depend on whether 
c  nfmode = 1,2 or 3, as described above.
      nfmode=0
c  irotkr, nprtkr: for irotkr = 1 calculate rotational kernels.
...
c
     ...
     *           ' Optional.'
      write(istdou,*) ' Grand summary on unit 11.'
      write(istdou,*) ' Short summary on unit 15.'
      write(istdou,*) ' Formatted summary on unit 16.'
      write(istdou,*) ' Eigenfunction on unit 4 (optional).'
      write(istdou,*) ' Rotational kernel on unit 12 (optional).'
      write(istdou,*) ' Gamma1 kernel on unit 13 (optional).'
      write(istdou,*) ' Printed output on unit istdpr, if not default'
c
      call ofiles
c
      call openf(idsgsm,'u','u')
      call openf(idsssm,'u','u')
c
...
c
      write(istdou,*) ' cntrd?'
      write(istdou,*) cntrd
      read(istdin,'(a)',end=85,err=85) cntrd
c
c  decode cntrd
c
...
c
        ...
        write(istdou,*) ifind,xmod,imlds,in,nprmod
        read(istdin,*,end=85,err=85) ifind,xmod,imlds,in,nprmod
        write(istdou,*) ' xtrnct,ntrnsf,imdmod?'
        write(istdou,*) xtrnct,ntrnsf,imdmod
        read(istdin,*,end=85,err=85) xtrnct,ntrnsf,imdmod
c
      end if
c
      if(ircosc.gt.0) then
c
        ...
        write(istdou,*) el,nsel,els1,dels,dfsig1,dfsig2,nsig1,nsig2
        read(istdin,*,end=85,err=85) 
     *    el,nsel,els1,dels,dfsig1,dfsig2,nsig1,nsig2
        write(istdou,*) ' itrsig,sig1,istsig,inomd1,itrds?'
        write(istdou,*) itrsig,sig1,istsig,inomd1,itrds
        read(istdin,*,end=85,err=85) itrsig,sig1,istsig,inomd1,itrds
        write(istdou,*) ' dfsig,nsig,iscan,sig2?'
        write(istdou,*) dfsig,nsig,iscan,sig2
        read(istdin,*,end=85,err=85) dfsig,nsig,iscan,sig2
        write(istdou,*) ' eltrw1, eltrw2, sgtrw1, sgtrw2?'
        write(istdou,*) eltrw1, eltrw2, sgtrw1, sgtrw2
        read(istdin,*,end=85,err=85) eltrw1, eltrw2, sgtrw1, sgtrw2
c
      end if
c
      if(ircrot.gt.0) then
c
        ...
        write(istdou,*)   irotsl, em, nsem, ems1, dems
        read(istdin,*,end=85,err=85) irotsl, em, nsem, ems1, dems
c
      end if
c
      if(irccst.gt.0) then
c
        write(istdou,*) ' cgrav?'
        write(istdou,*) cgrav
        read(istdin,*,end=85,err=85) cgrav
c
      end if
c
      if(ircint.gt.0) then
c
        ...
        write(istdou,*) iplneq,iturpr,icow,alb
        read(istdin,*,end=85,err=85) iplneq,iturpr,icow,alb
        write(istdou,*) ' istsbc,fctsbc,ibotbc,fcttbc?'
        write(istdou,*) istsbc,fctsbc,ibotbc,fcttbc
        read(istdin,*,end=85,err=85) istsbc,fctsbc,ibotbc,fcttbc
        write(istdou,*) 
...
c
        ...
        write(istdou,*) istdpr,nout,nprcen,irsord,iekinr
        read(istdin,*,end=85,err=85) istdpr,nout,nprcen,irsord,iekinr
        write(istdou,*) ' iper,ivarf,kvarf,npvarf,nfmode?'
        write(istdou,*) iper,ivarf,kvarf,npvarf,nfmode
        read(istdin,*,end=85,err=85) iper,ivarf,kvarf,npvarf,nfmode
        write(istdou,*) ' irotkr,nprtkr,igm1kr,npgmkr,ispcpr?'
        write(istdou,*) irotkr,nprtkr,igm1kr,npgmkr,ispcpr
        read(istdin,*,end=85,err=85) irotkr,nprtkr,igm1kr,npgmkr,ispcpr
...
c
     ...
     *    itssol,idgtss,moddet,iprdet,npout
        ...
        write(istdou,*) imstsl,imissl,imjssl,idgnrk
        read(istdin,*,end=85,err=85) imstsl,imissl,imjssl,idgnrk
c
      end if
c
c
...
c
      if(iscan.gt.1) then
	if(itrsig.gt.1) then
	  write(istdou,105) iscan, itrsig
	  if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,105) 
     *      iscan, itrsig
	  itrsig = 1
        end if
	if(istsig.gt.1) then
	  write(istdou,107) iscan, istsig
	  if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,107) 
     *      iscan, istsig
        end if
      end if
c
      if(istsig.gt.1.and.mod(itrsig,2).eq.0.and.nsel.gt.1) then
	write(istdou,110) istsig, itrsig, nsel
	if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,110) 
     *    istsig, itrsig, nsel
	nsel = 0
      end if
c
      if(istsig.gt.1.and.itrsig.eq.0) then
	write(istdou,112) istsig
	if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,112) istsig
	go to 6
      end if
c
      if(itsord.eq.1.and.iabs(itrsig).lt.2) then
	write(istdou,114) itrsig
	if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,114) itrsig
	itsord = 0
      end if
...
c
      if(eps.le.epsprc) then
        eps=1.1d0*epsprc
        write(istdou,120) eps
        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,120) eps
      end if
c
c  test for prohibiting Richardson extrapolation if mdintg = 2 or 4
c
c  removed on trial 18/1/91
c
c..      if((mdintg.eq.2.or.mdintg.eq.4).and.iriche.eq.1) then
c..        iriche=0
c..        write(istdou,934)
c..        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,934)
c..      end if
c                   -----------------------------------
c  write exec
c  **********
...
c
	...
	call openfc(itrds,itrdsp,'o',ctrdsf(itr))
      ...
      if(nfmode.ge.1) call openfc(4,idsmdp,'u','u')
      if(irotkr.eq.1) call openfc(12,idsrkp,'u','u')
      if(igm1kr.eq.1) call openfc(13,idsgkp,'u','u')
c
...
c
      nwmod=0
c
c  in first run read model
c
      if(frsrun.or.ifind.ge.0) then
c
        xmod1=xmod
c
...
c
        ...
        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,920) 
     *    ntrnct,nfit
      end if
c
      xfit1=x(nfit)
...
c
     ...
     *    iord1, iord2, dsolrc, dsigr
      else
	istatus=5
	isigcv=1
        write(istdou,120) iord1, iord2, dsolrc, dsigr
        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,120) 
     *    iord1, iord2, dsolrc, dsigr
      end if
      return
