...
c..      write(6,*) 'perfac, etc.',perfac,sig,epsufl
...
c
          write(istdou,125) n,x(n),(aa(i,n),i=1,6)
...
c
      if(irotkr.eq.1) call rotker(idsrkr,x(nw1),y(1,nw1),aa(1,nw1),
...
c
      if(nfmode.ge.1.and.isolcv.ne.-1) then
c
...
c
     ...
     *      n=nw1,nn)
	  ...
	  if(nfmesh.ne.0) then
	    if(istdpr.gt.0) then
              write(istdpr,*) 'nfmesh =',nfmesh
              write(istdpr,*) 'write mode mesh, nw1, nn =',nw1, nn
	    end if
...
c
      if(ispcpr.ne.0) call spcout(x,y,aa,data,nn,iy,iaa,ispcpr)
c
   80 continue
c
      return
  ...
  125 format(' ***** negative energy factor. n, x, aa(1-6) =',
     *  i5,f10.5,1p6e11.3)
