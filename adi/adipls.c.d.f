...
c
      ...
      logical fulmod,frsrun,radial,nonrad,nscfil
      character tail*5, cntrd*50, ctrdsf*1, in_stat*2, out_stat*2
      character*80 file, filess, obsrot_file, strcompr
c
...
c
c  test for suppressing output to istdpr, idsgsm, idsrkr and idsssm
c
      if(istdpr_in.le.0) then
        istdpr=0
        idsgsm=0
        idsssm=0
        idsrkr=0
...
c
     ...
     *    '' --------------------------------------------------'')')
c
c  reset original istdpr
c
c..     write(istder,*) '#D# Before first exit from adipls'
        istdpr=istdpr_ori
        return
c
      end if
...
c
      ...
      if(idsssm.gt.0) call openfc(idsssm,idsssp,out_stat,'u')
...
c
   90 if(idsgsm.gt.0) call close_file(idsgsm)
      if(idsssm.gt.0) call close_file(idsssm)
c
      if(idsefp.gt.0) call close_file(idsefn)
      if(idsrkp.gt.0) call close_file(idsrkr)
      if(idsgkp.gt.0) call close_file(idsgkr)
c
...
c
      if(istdpr.ne.istdpr_def.and.istdpr.gt.0) then
        write(istdou,'(/''Closing istdpr ='',i4/)') istdpr
c
c  test if this is a scratch file and requires special treatment
c
        if(nscfil(istdpr)) then
          close(istdpr)
        else
          call close_file(istdpr)
        end if
        istdpp=-1
      end if
      return
      istdpr=istdpr_ori
      write(istdou,'(/'' Restore istdpr to'',i3)') istdpr
...
c
	...
	if(istdpr.gt.0) write(idslog,100) file(nmlds),file(ndsgsm)
      end if
c
