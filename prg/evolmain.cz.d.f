...
c
c  Modified 14/10/04, preparing for repeated calls with parameter
...
c
c  Modified 13/7/05 to use real xmdtrl, possibly flagging for
...
c  if iddgm1 gt 0.
c
c  Modified 30/10/06, resetting flsatm from trial for init_param .ge. 2
c  (rather than .gt. 2) ********* Might deserve a little check.
c
...
c
      parameter(naztmx = nspcmx+3, iy = 2*ivarmx+1, 
...
c
     ...
     *  lastmd, concor, nscfil
      character*16 pmodu3
      character*80 file, filess, trailer_par, trailer_par_seq, 
     *  header_par, settrailer, strcompr
...
c
     ...
     *  '' Entering evolmain''/)')
c
      write(istdou,*) 'i_paramset, init_paramset', 
     *  i_paramset, init_paramset
c
...
c
     ...
     *  '0. Set init_paramset to', init_paramset
c
      write(istdou,101) iversn, invers
      if(istdpr.gt.0.and.istdou.ne.istdpr) write(istdpr,101) 
     *  iversn, invers
c
c  ******************************************************************
c
...
c  (for full precision), if iducen .gt. 0. If included, use 
c  iducen = 19, as default
      iducen=-1
...
c           if xmdtrl .ge. 10000 use last model on file.
...
c  Non-integral xmdtrl flags linear interpolation betwee models
c  no nmdtrl = int(xmdtrl) and nmdtrl+1
...
c  For nrpeat = -1, do not repeat stretching, but allow large
c  timestep also in the second timestep (to compensate for the
c  change in the model between the unstretched initial model and
c  the stretched next model).
      nrpeat=1
...
c  If istosc .ge. 10, set oscillation (and gong) variables also for
c  the computation of the evolution sequence when calling with 
c  parameter passing from set_modage.
      istosc=0
...
c
	...
	i_param=1
c
	if(istdpr.gt.0) istdpr=istdpr_ori
c
...
c
        init_paramset = 1
        if(istdpr.gt.0) write(istdpr,*) 
     *    '1. Set init_paramset to', init_paramset
        ierr_param=0
	go to 95
c
      end if
...
22000 continue
c
c  augment init_paramset. Note that init_paramset .ge. 2 is used
c  for setting trial flsatm from trial model below.
c
      init_paramset = init_paramset + 1
      if(istdpr.gt.0) write(istdpr,*) 
     *  '2. Set init_paramset to', init_paramset
c
...
c
      istdpr=istdpr_ev
      if(istdpr.gt.0.and.istdpr.ne.istdpr_def) then
	write(istdou,'(//'' Change istdpr to'',i3/)') istdpr
...
c
	...
	if(init_paramset.eq.0) then 
	  i_param=1
          init_paramset=1
          if(istdpr.gt.0) write(istdpr,*) 
     *      '3. Set init_paramset to', init_paramset
        else
	  i_param=2
...
c
	...
	agemxs=syear*agemax
	agefin=1.d23
        write(istdou,'(//'' Maximum age set to '',1pe13.5/)') 
     *    agemax
	if(istdpr.gt.0.and.istdpr.ne.istdou) 
     *    write(istdpr,'(//'' Maximum age set to '',1pe13.5/)') 
     *    agemax
      else
	agemxs=1.d33
      end if
...
c
      if(istdpr.gt.0) write(istdpr,*) '1. Surface X: ', y(5,1)
c
      if(icry.lt.0) then
	if(i_paramset.eq.0) then
          go to 20
...
c
c  test for resetting xxh in case with parameter setting
c
      if(ix.eq.1.and.i_paramset.gt.0) xxh=datmod(2)
c
...
c  Note: the variable zres is introduced as it may possibly be
c  reset in sethvz
c
      zres=z
      call sethvz(x,y,nnt,iy,datmod,ndtmod,zres,isetzh,idshvz)
c
c  with parameter setting, reset z as set up in sethvz, for 
c  storing in par_z
c
      if(i_paramset.gt.0) then
	if(istdpr.gt.0) write(istdpr,'(/'' z reset to'',f10.5,
     *    '' after call of sethvz''/)') zres
	z=zres
      end if
c
...
c  later.)
c
      if(i_paramset.gt.0.and.init_paramset.ge.2) then
	flsatm = datmod(140)
	if(istdpr.gt.0) write(istdpr,*) 
...
c
c  reset parameters stored in commons, following setups from trial
c
      if(i_paramset.gt.0) then
	i_param=1
	call res_param(i_param,
     ...
     *    nt, icsove, icsovc, isprot, icmout, istosc, isetos)
c
c  reset file-name trailer
c
        trailer_par=strcompr(settrailer(am, z, icase_trailer, 
     *    xseq_trailer, itcase))
	if(xseq_trailer.lt.0) trailer_par_seq=trailer_par
c
c  only print parameters for evolution sequences
c
	if(istdpr.gt.0.and.nt.gt.0) call print_par(header_par)
      end if
c
c  test for opening output files
c
...
c
     ...
     *    istrtc, epsr, wx, wpt, wr, wxh, wx3, wdgr, iwdgrd, tail
        ...
        write(istdpr,*)  wmshos,drmsos,wmshcc,dqlmsc,qmscmn,qmscmx,tail
...
c
c  Entry point after setting trial and resetting pnd printing parameters
c
c  -----------------------------------------------------------------------
c
   48 continue
...
c
c  possibly set flag for allowing large timestep in second timestep
c
      if(nrpeat.lt.0) then
        nrpeat=0
        ifr2dt=1
        if(istdpr.gt.0) write(istdpr,
     *    '(/'' Allow large time step in second timestep''/)')
      else
        ifr2dt=0
      end if
c
...
c
      zres=z
      call sethvz(x,y,nn,iy,datmod,ndtmod,zres,isetzh,idshvz)
c
...
ct..	write(6,'(a/(i5,1p7e10.3))') 'Before entering conmxc, n, x, y',
ct..     *    (n,x(n),(y(i,n),i=1,6),n=1,nn,20)
...
c..      write(6,*) 'ntime =',ntime,'  in1, in =',in1, in
...
ct..      write(6,'(a,i5,1pe13.5,i7,e13.5)') 
ct..     *  ' Before entering tnrkt, nmxcor, qmxcor, nmxcp, qmxcp =',
ct..     *  nmxcor, qmxcor, nmxcp, qmxcp
c
      iter=it
c
...
c
      if(istdpr.gt.0) then
        if(itndgn.ne.1) then
          write(istdpr,112) iter,(ea(k,1),k=1,ii),eam
        else
c
...
c
      if(idgtnr.ge.1.and.iter.le.idgtnr.and.istdpr.gt.0) then
        write(istdpr,352)
        do 59 n=1,nn
...
c
ct..      write(6,*) 'in test in evolmain, inmixc, inentc =', inmixc, inentc
...
c
	  if(irnucy.eq.1) then
            ...
            write(istder,*) 'Stop time0, no convergence in evolmain'
	    go to 95
	  else
	    irnucy=1
            nucy1=1+nit/2
...
c
	  ...
	  redfac=1.
          if(istcon.gt.0) then
	    write(istder,*) 'Stop 2d, no convergence in evolmain'
	    go to 95
	  endif
	  if(iheccs.ne.0) then
	    write(istder,*) 'Stop. Convergence problems'
	    go to 95
	  endif
	else 
c
...
c
      if(istdpr.gt.0) write(istdpr,*) 'In evolmain, xmxrhe =',xmxrhe
      do 61010 n=1,nn
...
c
c  test for allowing large timestep
c       
        if(ifr2dt.eq.1.and.ntime.eq.2) initdt=1
c
...
c
        if(istdpr.gt.0) write(istdpr,*) 
     *    'X reset from', xhsprv,' to xhsopc outside n =',nres
      end if
c
...
c
        ...
        if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,171) 
     *    agey,agefin
        go to 87
      end if
c
...
c  test for excessive number of iterations
        if(itlr.ge.nitlr) then
          ...
          if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,172) nitlr
        else
...
c
          ...
          if(iterlr.le.2.or.iterlr.ge.5) then
            ...
            if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,175) 
     *        dalfa,dxxh,alfa,xxh
          else
            write(istdou,176) dalfa,dxxh,sbcfct,xxh
            if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,176) 
...
c  scan modes on file
	...
	if(istdpr.gt.0) 
     *  write(istdpr,*) 'Close idsevl'
        close(idsevl) 
	if(istdpr.gt.0) 
     *  write(istdpr,*) 'Open idsevl for scan'
	idsev1=0
 ...
 1002 format(//' **** error in evolution programme.'/
  ...
  190 format(//' ***** error. ipartr = 1, but trial model provides',
     *  ' no data')
      ...
      subroutine test_eos(x,y,z,nn,iy,tl_test)
c
c  EOS test around interesting point
c
      implicit double precision(a-h, o-z)
      logical nosd, notd
      dimension x(*), y(iy,*)
...
c
      dtlmin=1.d10
      do n=1,nn
	dtl=abs(tl_test-y(3,n))
	if(dtl.lt.dtlmin) then
	  n_test=n
	  dtlmin=dtl
        end if
      end do
c
      ...
      zh0=z
      yh0=1.d0-xh0-zh0
      write(6,*) '#D# test of EOS centered at', fl0, tl0, xh0, zh0
      write(6,*) '#D# sfl, stl, dsfl, dstl',sfl, stl, dsfl, dstl
c
c  scan
c
      idgeos_orig=idgeos
	...
	do j=1,nsfl
	  fl=fl0-sfl+dsfl*(j-1)
          call eqstf(fl,tl,xh0,yh0,zh0,nosd,notd)
	end do
      end do
      idgeos=idgeos_orig
      return
