...
c
c     sub needs to be passed in, to calculate ndata
c
      implicit none
...
c     internal
      integer i, j, isb(nobs), ndata
      double precision dx, dp(ma), ymoda(nobs), ymodb(nobs), rx(ma)
...
c      
      ...
      enddo
c
c     call first to avoid any numerical issues..
      age_tmp=x(5)
      x(5)=1.d0
      call evalmodel (ma, x, nobs, ndata, ymod)
      x(5)=age_tmp
      ...
      do i = 1, ma
        if (ia(i).ne.0) then 
          ...
          do j = 1, ma
           ...
           enddo
           rx(i) = x(i) + dx
           call evalmodel (ma, rx, nobs, ndata, ymodb)
           rx(i) = x(i) - dx
           call evalmodel( ma, rx, nobs, ndata, ymoda)
           do j=1,nobs
              if (sub(j).ne.0) then 
                 dyda(j,i) = ((ymodb(j) - ymoda(j)))/(2.d0*dx)
             endif
          enddo
        endif
      enddo
...
c
      ...
      do j =1, nobs
         if (sub(j).ne.0) then
            dyda(j,5) = dyda(j,5)*cc(5)
         endif
      enddo

      call evalmodel (ma, x, nobs, ndata, ymod)
c     print *,'************* after evalmodel *****'
c      print *,x(1),1.0d-x(2)-x(3),x(3),x(4),x(5)
c      print *,ymod(4),ymod(5)
...
c     call the surface for the surface effects 
c     to calculate the modified frequencies :  
