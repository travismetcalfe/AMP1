...
c
c  Modified 4/11/05, to allow changing the number of Z tables used
c  in the interpolation. Note that number of tables is read on
c  first input line.
c
...
c
      data init/0/
      data indx, nindx  
     *   /101,  102, 103, 111, 112, 113, 121, 122, 123, 9/
      data cindx        /'data.Z2','data.Z18', 'data.Z1',
     *  '2001.Z2','2001.Z18','2001.Z1',
     *  '2005.Z020','2005.Z018','2005.Z010'/
c
      save
c
      idiag=0
...
c
	if(istdpr.gt.0)
     *    write(istdpr,'(/''Enter:''/
     *      '' Number of Livermore tables.''/
     *      '' File names for tables''/
     *      '' Z values of tables'')')
	read(istdin,*) nztab
	read(istdin,'(a)') (finliv(i),i=1,nztab)
	read(istdin,*) (zinliv(i),i=1,nztab)
c
c  set index number
c
	ivteos = -1
...
c
   25   continue
	if(ivteos.eq.-1) then
          ivteos=100
	  if(istdpr.gt.0) then
	    write(istdpr,105) (finliv(i),zinliv(i),i=1,nztab)
	  end if
c
	else if(istdpr.gt.0) then
	  write(istdpr,107) (finliv(i),zinliv(i),i=1,nztab)
c
        end if
c
c  reset ivteos, possibly to flag for several tables
c
	if(nztab.gt.1) ivteos=ivteos+1000*(nztab-1)
	if(istdpr.gt.0) write(istdpr,106) ivteos
      end if
c
c  ********************************************************************
c
...
c  set commons from s/r eqstf to zero
  ...
  105 format(//
     ...
     *  '       File names: '/(a70,' Z =',f6.3))
  106 format(' Version number set to',i5)
  107 format(//' EOS data to be read from files:'/(a70,' Z =',f6.3))
