...
c
c  Version allowing modification of G (cgrav)
c
c  Date: 4/4/03
c
c
...
c
     ...
     *    'Enter file name for Livermore tables'
	read(istdin,'(a)') finliv
c
c  set index number
c
        do 20 i=1,nindx
        if(index(finliv,cindx(i)).gt.0) then
	  ivteos=indx(i)
	  go to 25
        end if
   20   continue
c
        ivteos=100
	if(istdpr.gt.0) write(istdpr,105) finliv,ivteos
   25   continue
c
      if(idiag.gt.0.and.istdpr.gt.0) write(istdpr,107) finliv, ivteos
c
      end if
c
c  ********************************************************************
...
c  set commons from s/r eqstf to zero
      ...
      call zero(ccc3,10)
c
c  read in new value of cgrav
c
      if(istdpr.gt.0) write(istdpr,*) 'Enter new value of cgrav'
      read(5,*) cgravn
      write(istdou,130) cgrav, cgravn
      if(istdpr.ne.istdou.and.istdpr.gt.0) 
     *  write(istdpr,130) cgrav, cgravn
      write(99,130) cgrav, cgravn
      cgrav=cgravn
      return
  100 format(//2x,75('*')//
  ...
  107 format(//' EOS data to be read from file:'/1x,a/
     *  ' Version number set to',i5/)
  110 format(/' variable   old value    current, consistent value'/)
  120 format(a10,1p2e18.7)
  130 format(//' ***** Warning. cgrav changed from',1pe15.7,
     *  ' to',e15.7)
      end
