...
c..      parameter (mx=5,mv=10,nr=169,nt=191,nztab=2)
c..      character*3 fixedTP
c..      data fixedTP/'no'/  ! 'yes' for fixed T6,P; 'no' for fixed T6,rho
c..      common/eeoszt/esact,eoszt(mv,nztab)
...
c..      r=rhoofp (x,t6,p,irad)  ! calculate density (r) for fixed P.
c..      endif
c..      call esac(x,t6,r,iorder,irad) ! calc EOS; use r from rhoofp
c..      write (istdou,'("eos=",9e14.4)') (eoszt(i,iztab),i=1,9)
c..c                                            if fixedTP='yes'
c..      end
c..
c*************************************************************************
...
c
     ...
     . ,dfs(nt),dfsr(nr),m,mf,xa(mx)
      ...
      common/eeoszt/esact,eoszt(mv,nztab)
      common/ctablv/ iztab,zinliv(nztab),dummy(nztab)
c
...
c__________
  ...
  123 continue
     ...
     x ," using")')
        if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,
     *    '("Z does not match Z in EOSdata files you are"
     x ," using")')
        stop 'in esac-liv'
      endif
   ...
   45 continue
     ...
     x    ,epl(mi,it,ir),xx(mg),xx(mh),xx(mi))
          if(esk(it,ir) .gt. 1.e+20.and.istdpr.gt.0) then
            write(istdou,'(" problem it ir,l3,k3,iq,ip=", 6i5)') it,ir
     x      ,l3,k3,iq,ip
            write(istdou,'(3e12.4)')  (epl(ms,it,ir),ms=mg,mg+2)
          endif
...
c
  ...
  124 continue
      ...
      eoszt(iri(4),iztab)=eoszt(iri(4),iztab)*moles*aprop/tmass
      endif
      return
...
c..... The purpose of this subroutine is to read the data tables
      ...
      character*1 blank
      character*80 finliv
      common/ctablv/ iztdum,zdummy(nztab),finliv(nztab)
      common/aaeos/ q(4),h(4),xxh
      common/aeos/  xz(mx,mv,nt,nr,nztab),  
     . t6list(nr,nt),rho(nr),t6a(nt),esk(nt,nr),esk2(nt,nr),dfsx(mx)
     . ,dfs(nt),dfsr(nr),m,mf,xa(mx)
...
c..       open(ids, FILE='EOS2001data')
     ...
     *     '("problem with data files: X=",f6.4," density=",
     ...
     x t6list(jcs,i),alogr(jcs,i),(xz(m,index(iv),i,jcs,iztab),iv=1,9)
    4 continue
      enddo
      read(ids,'(a)') blank
      read(ids,'(a)') blank
...
c***********************************************************************
      ...
      common/eeoszt/esact,eoszt(mv,nztab)
      common/beos/ iri(10),index(10),nta(nr),zz(mx)
      common/ctablv/ iztab,zdummy(2),dummy(2)
c
c  common defining standard input and output
c
      common/cstdio/ istdin, istdou, istdpr, istder
...
c-----Calculate EOS without radiation correction
      ...
      chir=eoszt(iri(5),iztab)*eoszt(iri(1),iztab)/pt
      chitt=(eoszt(iri(1),iztab)*eoszt(iri(6),iztab))/pt
      cvtt=(eoszt(iri(4),iztab)*molenak/tmass)
...
c---- Calculate EOS with radiation calculation
      ...
      chir=eoszt(iri(5),iztab)*eoszt(iri(1),iztab)/pt
      chitt=(eoszt(iri(1),iztab)*eoszt(iri(6),iztab)+4.*pr)/pt
...
c     gam3pt(jcs,i)=gam1t(jcs,i)/gam2pt(jcs,i)
      cvtt=(eoszt(iri(4),iztab)*molenak/tmass+4.*er/t6)
...
c    x  /molenak
      ...
      eoszt(iri(4),iztab)=cvtt
      eoszt(iri(5),iztab)=chir
      eoszt(iri(6),iztab)=chitt
...
c       eq. 16.16 Landau&Lifshitz (SEE line labeled DIRECT)
      eoszt(iri(7),iztab)=eoszt(iri(7),iztab)+gam1t-gam1t_norad
      eoszt(iri(8),iztab)=eoszt(iri(8),iztab)+gam2pt-gam2pt_norad
      eoszt(iri(9),iztab)=eoszt(iri(9),iztab)+gam3pt-gam3pt_norad
      return
      end
c********************************************************************
