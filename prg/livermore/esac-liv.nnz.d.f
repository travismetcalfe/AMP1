...
c
c  Modified 6/11/95, to separate part of common/a/ which is set
c  by data statement
c
c  Modified 24/8/96, doing computations in double precision
c  Modified 24/8/96, assuming file name for tables set in
c  common/ctablv/
c
...
c
     ...
     . ,dfs(nt),dfsr(nr),m,mf
      common/a1/ xa(mx)
      common/b/ iri(10),index(10),nta(nr),izz,zz(mx)
...
c
      ...
      data aprop/83.1446304/
      blank=' '
      if (iorder .gt. 10 ) then
         write (istdou,'(" Iorder cannot exceed 10")')
         if(istdpr.ne.istdou.and.istdpr.gt.0) write (istdpr,
     *     '(" Iorder cannot exceed 10")')
      endif
...
c..... read the data files
     ...
     *      '(" ***** Warning: requested z=",f10.6,
     *      " EOSdata is for z=",f10.6)') ztab,z      
c..        stop
        endif
...
c
  ...
  124 continue
   ...
   62 write(istdou,'(" T6/LogR outside of table range (62)")')
      write(istdou,'(" T6, logR =",1p2e13.5)') slt, slr
      if(istdpr.gt.0.and.istdpr.ne.istdou) then
        write(istdpr,'(" T6/LogR outside of table range (62)")')
        write(istdpr,'(" T6, logR =",1p2e13.5)') slt, slr
        write(istdpr,*) 'nt, t6a(1), t6a(nt), slt',
     *    nt, t6a(1), t6a(nt), slt
        write(istdpr,*) 
     *  '  nr, rho(1), rho(nr), slr',nr, rho(1), rho(nr), slr
      end if
      stop 'esac-liv'
...
c
     ...
     . ,dfs(nt),dfsr(nr),m,mf
      ...
      common/e/ esact,eos(mv)
      common/ee/ epl(mx,nt,nr),xx(mx)
      common/eee/moles(mx),xin(mx),tmass(mx),icycuse(mx,nr),irhogr,
     *  rhogr(mx,nr),frac(mx,6),alogr(nr,nt)
c
c  common defining standard input and output
c
      common/cstdio/ istdin, istdou, istdpr, istder
...
c
      ...
      read (ids,'(3x,f6.4,3x,f6.4,11x,f10.7,17x,f10.7)')
     ...
     *     '("problem with data files: X=",f6.4," density=",
     x      e14.4)') xin(m), rhogr(m,jcs)
      ...
      read (ids,'(f9.5,1x,f6.2,3e13.5,e11.3,6f8.4)')
   ...
   13 dfsr(i)=1./(rho(i)-rho(i-1))
      ...
      enddo
c..       call system ('compress EOSdata')
      close(ids)
      return
      end
c
...
c******************************************************************8
      ...
      data (eion(i),i=1,6)/-3388.637,-1970.918,-1431.717,-993.2303,
     x -76.2315,-15.29409/
...
c
     ...
     x unitfold/0.9652/, c/2.9979245e+10/, sigma/5.67051e-5/
     x , sigmac/1.8914785e-15/, sigmacc/1.8914785e-3/, aprop/83.1446304/
...
c     data (nra(i),i=1,nt)/13*77,4*76,4*74,4*72,4*70,2*68,2*67,
c    x 2*66,2*65,2*64,
c    x 2*63,2*61,2*60,2*59,2*58,2*57,2*55,2*54,4*53,4*52,4*51,
c    x 8*49,4*48,8*47,4*46,8*45,60*44,8*37/
      data (nra(i),i=1,nt)/17*121,2*120,2*118,4*116,2*115,2*114,
     x 2*113,2*111,2*110,2*109,2*108,
     x 4*106,2*104,2*103,2*102,2*101,
     x 4*99,4*98,4*97,4*96,8*94,4*93,
     x 8*92,4*91,8*90,60*88,8*74/
      rat=sigmacc
      pr=0.0
...
c     stop
      if(istdpr.gt.0) write (istdpr,'("pnr, pmax,pmin=",3e14.4)') 
     *  pnr,pmax,pmin
      endif
...
c
     ...
     x 2*99,91,87,79,75,67,67,63,59,55,53,51,51,49,47,45,45,41,41,39,
     x 37,35,33,31,29,29,25,25,21,21,17,17,13/
