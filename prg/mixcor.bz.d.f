...
c
      ncrlim=nn+1
      if(istdpr.gt.0) 
     *  write(istdpr,*) ' In mixcor, ncrlim =',ncrlim
c 
...
c
   37 if(imixc0.eq.1.or.addvar(1,nlim).le.0) then
        ...
        pl=addvar(1,nlim)
        drad=fdradp(x(nlim),y(1,nlim),pl,zh(nlim),ak,akr,akt,akx)
      end if
      qlim=10.d0**x(nlim)
...
c
      if(imixc0.eq.1.or.addvar(1,n).le.0) then
        ...
        pl=addvar(1,n)
        drad=fdradp(x(n),yn,pl,zh(n),ak,akr,akt,akx) 
      end if
      if(idiffc1.eq.0) then
        ddad=drad-dad(1)
      else
...
c
     ...
     *    ddad.le.0.and.ddads(is-1).lt.ddad) then
          ...
          if(frmxc.le.2) then
            nmxcor=n
	    if(istdpr.gt.0)
     *        write(istdpr,120) nmxcor, is, ddads(is-1), ddad
            qlmxc=frmxc*x(nmxcor)+(1-frmxc)*x(nmxcor-1)
            qmxcor=10.d0**qlmxc
            rlmxc=frmxc*y(1,nmxcor)+(1-frmxc)*y(1,nmxcor-1)
            rmxcor=(10.d0**rlmxc)/10.d0**y(1,1)
...
c
     ...
     *        log10(ak),yn(4),ddad
	  ...
	  if(istdpr.gt.0) write(istdpr,122) iextrp, nmxcor, is, is1,
     *      ddads(is1-1), ddads(is1)
          qlmxc=frmxc*x(nmxcor)+(1-frmxc)*x(nmxcor-1)
          qmxcor=10.d0**qlmxc
          rlmxc=frmxc*y(1,nmxcor)+(1-frmxc)*y(1,nmxcor-1)
          rmxcor=(10.d0**rlmxc)/10.d0**y(1,1)
...
c
     ...
     *        log10(ak),yn(4), ddad
	  ...
	  is1=is-1
          frmxc=ddads(is-2)/(ddads(is-2)-ddads(is-1))
	  if(istdpr.gt.0) write(istdpr,125) iextrp, nmxcor, is, is1,
     *      ddads(is1-1), ddads(is1)
          qlmxc=frmxc*x(nmxcor)+(1-frmxc)*x(nmxcor-1)
          qmxcor=10.d0**qlmxc
          rlmxc=frmxc*y(1,nmxcor)+(1-frmxc)*y(1,nmxcor-1)
