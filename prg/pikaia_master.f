      subroutine pikaia_master
c======================================================================
c     driver program for pikaia.f
c======================================================================
      implicit none
      integer n, seed, i, status, iargc
*** NUMBER OF PARAMETERS !!!
      parameter (n=4)
      real ctrl(12), x(n), f, userff
      character*80 seed_string
      external userff
c -------------------------------------------
...
c -------------------------------------------
      seed = 123456
      if (IARGC() .GT. 0) then
        call getarg(1,seed_string)
        read(seed_string,*) seed
      endif
...
*** crossover probability
      ctrl(4)=0.85
*** mutation mode (5/6=uniform+creep variable F/D)
      ctrl(5)=6
*** initial/min/max mutation rate
      ctrl(6)=0.005
      ctrl(7)=0.001
      ctrl(8)=1./(n*ctrl(3))
*** relative fitness differential
      ctrl(9)=1.00
*** reproduction plan (1=full gen.rep.)
      ctrl(10)=1
*** elitism flag (1=keep best)
      ctrl(11)=1
*** output verbosity level 0/1/2
      ctrl(12)=2
c
c     Now call pikaia
      call pikaia(userff,n,ctrl,x,f,status)
c
c     Print the results
      write(*,*) ' status: ',status
      write(*,*) '      x: ',x
...
c
      ...
      if (file_exists) then
         restarted=.TRUE.
	 write(*,*)
	 write(*,*) "initializing with restart.dump"
         open(55,file='restart.dump',status='old')
	 read(55,'(F8.6,4(1x,I3.3))') mrate,sgen,npar,npop,newtot
	 write(*,'(F8.6,4(1x,I3.3))') mrate,sgen,npar,npop,newtot
	 do pop=1,npop
	    read(55,'(2(I3.3,1x),4(F4.2,1x),E13.7)') ifit(pop),
     +      jfit(pop),(newph(par,pop),par=1,npar),fitns(pop)
	    write(*,'(2(I3.3,1x),4(F4.2,1x),E13.7)') ifit(pop),
     +      jfit(pop),(newph(par,pop),par=1,npar),fitns(pop)
            do par=1,npar
...
c     Compute initial (random but bounded) phenotypes
      do 1 ip=1,np
         do 2 k=1,n
            oldph(k,ip)=urand()
    2    continue
cMPI  --- line below commented out for parallel version ---
cMPI     fitns(ip) = ff(n,oldph(1,ip))
    1 continue

cMPI  --- get fitnesses [in parallel] using caller's fitness function ---
...
c     Main Generation Loop
   22 do 10 ig=1,ngen
c
cRESTART skip forward to ig=sgen
c
         if (restarted) then
	    if (ig.lt.sgen) goto 10
	    if (ig.eq.sgen) goto 11
	 endif
...
c
         open(55,file='restart.dump',status='unknown')
	 write(55,'(F8.6,4(1x,I3.3))') pmut,ig,n,np,newtot
	 do pop=1,np
	    write(55,'(2(I3.3,1x),4(F4.2,1x),E13.7)') ifit(pop),
     +      jfit(pop),(newph(par,pop),par=1,n),fitns(pop)
	 enddo
         close(55) 
c
cRESTART reset mutation rate and then proceed normally
c
...
c     to replace)
c      if (ielite.eq.1 .and. ff(n,newph(1,1)).lt.fitns(ifit(np))) then
      if (ielite.eq.1) then
         do 1 k=1,n
