...
c
      ...
      if(istdou.ne.istdpr) write(istdou,101) iversn, invers
c
...
c              (1998; Rev. Mod. Phys., in the press)
...
c
      if(ipartr.eq.1.and.(itriab.eq.1.or.idatmd.eq.0)) then
        write(istdou,190) 
        if(istdou.ne.istdpr) write(istdpr,190)
        go to 20
      end if
c
...
c
      if(ivreos.ne.1) then
        if(modeeq.ne.0) then
          write(istdpr,1001) ivreos, modeeq
          if(istdou.ne.istdpr) write(istdou,1001) ivreos, modeeq
          modeeq=0
        end if
c
      else
...
c
        if(modeeq.eq.0) then
          ...
          write(istdpr,1002) modeeq
          if(istdpr.ne.istdou) write(istdpr,1002) modeeq
...
c
      if(istdpr.ne.istdou) call openfc(istdpr,istdpp,'u','f')
c
      write(istdpr,*) 'cntrd'
...
c
        ...
        iwrthv=iwrthp
        if(isethv.ne.3) write(istdpr,109) anhe0
        anhe0=6
      end if
c
...
c
      if(isethv.eq.3) then
        ...
        anhe0=8
c
        write(istdpr,110) az, anz0, anhe0
c
      end if
c
c  storage indices
...
c
   44 continue
c
      if(idgeng.eq.-3) write(6,*) 
     *  'before setting trial thte =',thte
c
c  trial solution
c  **************
c
...
c
	  ...
	  write(istdpr,148) file(nopfd)(1:75)
	else
c
          dtopfg(1)=fdgopl
c
...
c
        ...
        write(istdpr,104) patmos,alamop
c
      end if
      itscen=9
c..      write(iducen) itscen
...
c..      write(6,*) 'ntime =',ntime,'  in1, in =',in1, in
      if(idgbcs.ge.1) write(istdpr,445) bccoef
...
c
      ...
      if(istdpr.ne.istdou) write(istdpr,105) ntime,age,agey,dt,dty
c
...
c
      if(nxtst1.lt.nxtst2) then
        write(6,57099) 'y(in,.)',0,(n, 10.**x(n),y(in+4,n),
     *    y(in+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
        write(6,57099) 'y(in1,.)',0,(n, 10.**x(n),y(in1+4,n),
...
c
      if(nxtst1.lt.nxtst2) then
        write(6,57099) 'y(in,.)',1,(n, 10.**x(n),y(in+4,n),
     *    y(in+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
        write(6,57099) 'y(in1,.)',1,(n, 10.**x(n),y(in1+4,n),
     *    y(in1+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
      end if
...
c
      ...
      if(idgeng.eq.-3) write(6,*) 
     *  'before call of tnrkt thte =',thte
      write(6,*) 'Before call of tnrkt, iopatm =',iopatm
c
      if(iprhe4.ge.1) write(istdpr,'(a/(i5,1p5e15.7))') 
...
c..      if(ishfal.gt.0) call shftal(x,y(in1,1),nn,iy,1,initsh)
     ...
     .  kk,ka,kb,ki,nn,iy,ucy,ea,det,v,theta,dt,iter)
      ...
      if(irczms.eq.1.and.mod(iter-itczms,2).eq.0) then
        write(istdpr,'(/a/)') ' Before call of rczmsh'
        j=1
...
c
      if(itnprt.gt.0) write(istdpr,351) (n,x(n),(y(in11+j,n),j=1,6),
     .  n=1,nn,itnprt)
...
c
      if(idgtnr.ge.1.and.iter.le.idgtnr) then
        write(istdpr,352)
        do 59 n=1,nn
...
c
	...
	write(istdpr,59081) eam
	go to 60010
      else if(iter.gt.iterlm.and.eam.gt.2*eamp) then
	itwarn=itwarn+1
	write(istdpr,59080) eam,eamp
...
59008   xnew=xnew+y(in1+4,i)
        xnew=xnew/(n2-n1+1)
	write(istdpr,59091) y(in1+4,n), n, x(n), xnew
...
c
      write(6,*) 'in test in evolmain, inmixc, inentc =', inmixc, inentc
...
c
      if(nxtst1.lt.nxtst2) then
        write(6,57099) 'y(in,.)',2,(n, 10.**x(n),y(in+4,n),
     *    y(in+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
        write(6,57099) 'y(in1,.)',2,(n, 10.**x(n),y(in1+4,n),
     *    y(in1+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
      end if
c
      if(eam.lt.eps) then
...
60010 if(eam.gt.eps) then
        write(istdpr,150) eam, eps
        if(istdpr.ne.istdou) write(istdou,150) eam, eps
...
c
	  if(irnucy.eq.1) then
            write(istdpr,154)
            if(istdpr.ne.istdou) write(istdou,154)
...
c
	  ...
	  iqcrsp = 0
          write(istdpr,153) iqcrsp
          if(istdpr.ne.istdou) write(istdou,153) iqcrsp
	  inentc = 1
	  inmixc=1
	  redfac=1.
...
c
	  ...
	  if(iqcres.eq.1) then
	    ...
	    redfac=1.
            ...
            if(istdpr.ne.istdou) write(istdou,153) iqcres
	...
	call redtst(y(in,1),zk,y(in1,1),zk,dt,redfac,eam,ii,kk,nn,iy)
        write(istdpr,158) redfac
        if(istdpr.ne.istdou) write(istdou,158) redfac
	age=age+dt
        agey=age/syear
        dty=dt/syear
        write(istdou,105) ntime,age,agey,dt,dty
...
c
	...
	write(6,*) 'Call mixcor after completion of iteration'
...
c
      if(nxtst1.lt.nxtst2) then
        write(6,57099) 'y(in,.)',3,(n, 10.**x(n),y(in+4,n),
     *    y(in+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
        write(6,57099) 'y(in1,.)',3,(n, 10.**x(n),y(in1+4,n),
     *    y(in1+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
      end if
c
      if(itsxin.ge.1) then
...
c
      if(istdpr.ne.istdou) then
        ...
        write(istdou,113) iter,eam,y1,y4
      end if
c
...
c
      if(ivreos.ne.modeeq.and.ivreos.ne.9) then
        write(istdpr,125) ivreos, modeeq
        if(istdpr.ne.istdou) write(istdou,125) ivreos, modeeq
      end if
c
...
c
        write(istdpr,117)
c
	write(6,*) 'inc, nf,nl,frcf,frcl', inc, nf,nl,frcf,frcl
	call flush(6)
c
        do 713 j=1,inc
        nff=nf(j)
...
c
        frc=frcf(j)
        frc1=1-frc
        write(6,*) ' Convection, j, nff, nll, frc, frc1, in11 =',
...
c
      if(nxtst1.lt.nxtst2) then
        write(6,57099) 'y(in,.)',4,(n, 10.**x(n),y(in+4,n),
     *    y(in+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
        write(6,57099) 'y(in1,.)',4,(n, 10.**x(n),y(in1+4,n),
     *    y(in1+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
      end if
c
...
c
   ...
   83   continue
c
        write(6,*) 'X reset from', xhsprv,' to', xhsopc,
     *    ' outside n =',nres
      end if
c
...
c
	  ...
	  write(istdpr,*) 'Reset Y. Y_old, Y_new:',
...
c
      if(nxtst1.lt.nxtst2) then
        write(6,57099) 'y(in,.)',5,(n, 10.**x(n),y(in+4,n),
     *    y(in+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
        write(6,57099) 'y(in1,.)',5,(n, 10.**x(n),y(in1+4,n),
     *    y(in1+iyche4-1,n),n=nxtst1,nxtst2,ndxtst)
      end if
c
      go to 50
...
c
        ...
        if(istdpr.ne.istdou) write(istdpr,171) agey,agefin
        go to 87
      end if
c
...
c  test for excessive number of iterations
        if(itlr.ge.nitlr) then
          ...
          if(istdpr.ne.istdou) write(istdpr,172) nitlr
        else
...
c
          ...
          if(iterlr.le.2.or.iterlr.ge.5) then
            ...
            if(istdpr.ne.istdou) write(istdpr,175) dalfa,dxxh,alfa,xxh
          else
            write(istdou,176) dalfa,dxxh,sbcfct,xxh
            if(istdpr.ne.istdou) write(istdpr,176) dalfa,dxxh,sbcfct,xxh
          end if
          write(istdpr,177)
          go to 21001
c
...
c  last sequence is models ipmlr0+1, ..., ipmlr
     ...
     *      (bccoef(j),j=1,nbccf)
          if(i.gt.ipmlr0) k=2
          write(istdpr,182) i,datmod(22),datmod(24),datmod(25),pmodu3(k)
...
c
   90 continue
      if(ibegin.eq.0) then
	write(istdpr,159)
	if(istdou.ne.istdpr) write(istdou,159)
