...
c
      if(idgeng.gt.0) then
        write(6,*) 'Enter engenr with fl, tl, xx:',fl, tl, (x(i),i=1,4)
	write(6,*) 'icnocs, iheccs =', icnocs, iheccs
      end if
c
...
c
      ider1=nspect+3
      if(idgeng.eq.-2) write(6,*) 'nspec, nspect, ider1',
     *  nspec, nspect2, ider1
...
c
   ...
   11   ft(i,k)=0
	nohrct=.true.
	if(idgeng.gt.0) write(istdpr,*) 'No hydrogen reactions'
	go to 54
      end if
c
...
c
	...
	if(idgeng.ge.1) write(6,*) 'Setting xhe3eq.',
...
c
      ...
      if(idgeng.eq.-10) write(6,*)
     *  'epp(1),cpp,av,rhh,al(1,1),qe(1)',
     *  epp(1),cpp,av,rhh,al(1,1),qe(1)
      do 52 i=2,iderh
      depp=qe(i)
...
c..      idgeng=-10
      if(idgeng.eq.-10) 
     *  write(6,*) 'epp(1), epscno(1), epshec(1), epss',
     *  epp(1), epscno(1), epshec(1), epss
c
      eps(1)=epss
c
...
c
   ...
   85 ft(i,1)=rha1*dft1+fthcno(i)
      if(idgeng.eq.-42) then
	write(6,*) 'ider1 =',ider1
	write(6,*) 'fthcno:',(fthcno(i),i=1,ider1)
	write(6,*) 'ft(.,1):',(ft(i,1),i=1,ider1)
      end if
c
...
c
  ...
  128 eps(jj)=(-amm*st1(i)*st1(j)/epss+epp(1)*(amm*epp(i)*epp(j)
     .  +epp(jj))+epscno(1)*(amm*epscno(i)*epscno(j)+epscno(jj)))/epss
      if(idgeng.ge.1) then
	write(6,*) 'epp:',(epp(i),i=1,ider)
        write(6,*) 'epscno:',(epscno(i),i=1,ider)
        write(6,*) 'eps:',(eps(i),i=1,ider)
      end if
c
c..      idrprt = 0
c
...
c
	...
	write(istdpr,205) irnrat
	if(istdou.ne.istdpr) write(istdou,205) irnrat
	stop 'rnrate'
      end if
c
...
c
         ...
         write(*,*) 'Reaction rates in rnrate'
         do j=1,krnrat
            write(*,*) al(1,j)
         enddo
...
c
  ...
  220 format(' electron screening with zeta =',1pe13.5,
     *  '  Uw = ',e13.5)
c  221 format(' start setting lambda for k =',i3,'  kpar =',i3/
c     *  ' a(k), b(k), zzscr(k), t13 =',1p4e13.5)
...
c
	...
	write(istdpr,110) ieqhe3
	if(istdpr.ne.istdou) write(istdou,110) ieqhe3
	stop 'engcse'
      end if
c
      if(icnocs.eq.0) then
	...
	write(istdpr,120) icnocs
	if(istdpr.ne.istdou) write(istdou,120) icnocs
	stop 'engcse'
      end if
c
      nspec = nspcno + nspxx3
...
c
	...
	nsphec = 0
      ...
      if(idgeng.ne.0) write(6,*) 'nspcno, nspxx3, nspec, nspect',
...
c
     ...
     *  idsoov,idsgng,idsgsm,idstm1,idstm2,idstm3
c
c  common defining standard input and output
c
      common/cstdio/ istdin, istdou, istdpr
