...
c
      ...
      double precision Teff,L_Lo,T_obs,L_obs,Terr,Lerr,offset(200)
      ...
      double precision M_obs,Merr,M_H,Mresid
...
c
         ...
         read(55,*) num_obs
         do obs=1,num_obs+3
c            read(55,*,end=50) col1str,freq(obs),err(obs),mixed(obs)
...
c
            if (num_step .ge. ntree-5) then
               ...
               do obs=1,num_obs
                  best = 10000.
                  do calc=1,nobs_st
                     if(obs_st(1,calc) .eq. ell(obs)) then
...
c
     ...
     +             (csum_ind(3,nstep_ind)*csum_ind(3,nstep_ind)))
            M_H = log10(par_z/par_xxh)+1.61d0
...
c         a0 = ((f0 - r*fm)*float(num_rnu)) / sum_obs
...
c
            ...
            fcalc_sys = obs_st(3,mode) + sys
            weight = err(obs)
c            weight = sqrt(err(obs)*err(obs) + 0.1*sys*sys)
c            weight = sqrt(err(obs)*err(obs) + 0.25*sys*sys)
...
c
         chisq_r = sum_rsq/float(num_obs+nonseis-5)
...
c      call getenv("EPRGDIR",eprgdir)
      eprgdir="/home/travis/evolpack"
 3    format("13 '",a,"/opac/ghwd-v11.gn93_ax94'")
...
c                       #X
 7    format(" 3 1 2 ",f5.4,"  1 2 0 2  Model S  Liv05")
      ...
      write(55,'(A)') " .2490 1 1.0 2 5.000E+07 1 6 0    Model S"
      write(55,'(A)') " ,,,,0.51537,,,,,,,,,,"
      write(55,'(A)') " ,,,,,,,,,,,,,,,,,,"
      write(55,'(A)') " ,,,,,,,,,,,,,,,,,,"
c              #alpha
