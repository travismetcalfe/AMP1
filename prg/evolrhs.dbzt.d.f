...
c
      dimension yin(ifd),zk(1),f(ifd),fd(ifd,*),alam(ialam,*),
     ...
     .  icvflg(nnmax), asfdif(nspdmx), bsfdif(nspdmx),
     .  difsf(7), difsfp(7), difsfs(7,nnmax)
c
...
c
      ...
      data initrs /0/
c
c  initialize counting indices for convection-zone boundaries
c
      data nf, nl /6*0, 6*0/
c
...
c
c  initialize convective-envelope X and Z, with heavy-element
c  settling (for later test of stability problems at base of 
...
c
c  set temporary flag for tests at base of envelope
c
...
c
      if(iter.eq.0.and.n.eq.1) nxhzer=nn+1
c
...
c..      call zero(crxmnp,nspcmx)
   ...
   17   ft(j,2)=0
	...
	if(icqcdg.eq.1.or.idgeng.ge.1) then
          if(istdpr.gt.0) then
...
c..      if(idiffc1.gt.0.and.(iter.eq.0.or.iter1.ge.3)) then
      if(idiffc1.gt.0.and.idiftce.lt.1) then
c
...
c
c  test for convective instability due to composition change resulting
c  from settling in the vicinity of the base of the convective
c  envelope
c
      if(idiffus.eq.2.and.idiftce.ge.1.and.nl(1).gt.0
     *  .and.n.ge.nl(1).and.n.le.nl(1)+ndiftce) then
        if(iter.eq.0) then
          drce = dr*(1-rkx*(y(4)-xcep)/xcep-akz*(y(5)-zcep)/zcep)
        else
          drce = dr*(1-rkx*(y(4)-xce)/xce-akz*(y(5)-zce)/zce)
	end if
	if(dr.ge.dad(1).and.drce.le.dad(1).and.istdpr.gt.0) then
	  write(istdpr,'(''COMP. inst.; n, x, dad, dr, drce ='',
     *      i5,1p4e13.5)') n, x, dad(1), dr, drce 
	  if(idiftce.ge.2) dr=drce
	end if
      end if
      dac=dr
      ddrad=dr-dad(1)-dtxh
...
c
   ...
   23     cvvarf(i,inc)=frc*cvvar(i)+(1-frc)*cvvarp(i)
	  if(istdpr.gt.0) then
	    write(istdpr,*) 'cz start. inc, dcnp, dcn, frcf =',
     *      inc, dcnp, dcn, frc
...
c
	  if(istdpr.gt.0) then
	    write(istdpr,*) 
     *        'cz end. inc, dcnp, dcn, frcl =', inc, dcnp, dcn, 1-frc
	    write(istdpr,*) 'x, y:',x,(y(i),i=1,6)
	  end if
	  if(inc.eq.1) then
	    rczl = r
...
c
	if(inc.eq.1.and.idiffus.eq.2) then
c
c  reset convective-envelope X and Z 
c
	  if(iter.eq.0) then
	    xcep = y(4)
	    zcep = y(5)
...
c  NOTE: limit changed from 3.*dmrovs to 6.*dmrovs on 12/7/06
c
...
c
     ...
     *           (y(4+idcomp+j)-(velx(j,1,1)/amms)*y(3+j))
	  asfdif(j)=amm*qx*amms*amms
	  bsfdif(j)=-amm*qx*amms*velx(j,1,1)
...
c
   30 continue
c
c  test radiative temperature gradient for (later) resetting
c  of gradients in X and Z.
c
      if(n.eq.1) then
	do i=1,7
	  difsfp(i)=0
	...
	do i=1,7
      ...
      if(idiffus.eq.2.and.idiftce.ge.1.and.nl(1).gt.0
     *  .and.n.ge.nl(1).and.n.le.nl(1)+ndiftce.and.iter.gt.0) then
	derdr_min=0.1
	idiag_dr=0
        call dgrder(x, y, zz, yprt(1,n), f, asfdif, bsfdif, difx, 
     *     n, dr, rkr, rkt, rkx, akz, nl(1), derdr_min, idiag_dr, difsf)
c
c  test for resetting diffusion coefficients and rhs
c       
	if(idiftce.ge.2.and.n.gt.1.and.n.lt.nn) then
c
c  Attempt to reset to get properly positioned value
c  turned out to be extremely unstable. Drop for now.
c
          if(iter1.le.15) then
	    do i=1,7
c..	      difsf(i)=2.d0*difsf(i)-difsfp(i)
	      difsfs(i,n)=difsf(i)
	    end do
          else
	    if(difsf(1).gt.0.and.idiag_dr.gt.0.and.istdpr.gt.0)
     *        write(istdpr,*) 'n, difsf, difsfs(n)',
     *        n, difsf(1), difsfs(1,n)
            do i=1,7
	      difsf(i)=difsfs(i,n)
	    end do
          end if
...
c
	...
	csdifr(2*idcomp+2,n)=difsf(1)/(dmdr*dmdr)
      end if
c
...
c..      if(convx.or.(inmixc.ne.1.and.nmxcor.gt.0.and.qx.le.qmxcor)) then
     ...
     *  (convx.or.(inmixc.ne.1.and.nmxcor.gt.0.and.qx.le.qmxcor))) then
c
c  reset dzdy(2,4)
c
	dzdy(2,4)=0
...
c
 ...
 1510 format(1p6e12.4)
      ...
      subroutine dgrder(x, y, zz, yp, f, asfdif, bsfdif, difx, 
     *  n, dr, rkr, rkt, rkx, akz, nll, derdr_min, idiag, difsf)
c
c  To develop into a way to fix gradients to compensate for
c  heavy-element settling beneath convective envelope
c
c  Original version: 8/12/04
c
      implicit double precision(a-h, o-z)
      dimension y(*), zz(*), yp(*), f(*), asfdif(*), bsfdif(*), difx(*),
     *  difsf(*)
      dimension xxh(2), xxhp(2), xxhm(2), xxhpm(2), yyh(2), yyhm(2), 
     *  xxhn(2), yyhn(2),
     *  asfdfm(2), bsfdfm(2), alpha(2), alpham(2), difxm(2)
c
c  common giving fundamental constants
c
      common /fconst/ pi, ebase, amu, ame, clight, planck, boltzm, 
...
c
c  set relevant opacity derivatives
c
      rhopt = rho(3) - pt(3)*rho(2)/pt(2)
      rhopx = (rho(4) - pt(4)*rho(2)/pt(2))*amm*y(4)
      akpp = rkr*rho(2)/pt(2)
      akpt = rkt + rkr*rhopt
      akpx = rkx + rkr*rhopx
      akpz = akz
c
c  now we are ready to set derivatives for testing
c
      alpha(1) = dr*akpx/y(4)
      alpha(2) = dr*akpz/y(5)
c
...
c
      ilum=4+idcomp
      drder0 = dr*amm*((akpp+1)*f(2)+(akpt-4)*f(3)+f(ilum)-1)
      beta = - drder0
c
      if(idiags.ge.2) then
        drderx = dr*akpx*f(4)/y(4)
        drderz = dr*akpz*f(5)/y(5)
        drder  = drder0 + drderx + drderz
        write(istdpr,'(a,i5,1p2e15.7,4e13.5,6e11.3)') 
     *    'DGRDER',n,x,dr,drder0,drderx,drderz,drder, 
     *    akpp, akpt, akpx, akpz, f(4)/y(4), f(5)/y(5)
c
      end if
c
      xxh(1)=y(4)
      ...
      xxhp(2)=yp(5)
c
c  test for solving equations for new composition and setting
c  diffusion coefficient (eventually!)
c
c  for now initialize difsf to 0 for testing
c
      difsff=0.d0
c
      if(n.eq.nll) then
	do i=1,2
      ...
      else if(n.gt.nll) then
c
c  as a rather desparate test make sure that betam is sensible here
c
	if(betam/beta.lt.0.5) then
	  write(istdou,
     *      '(//'' ***** Problems with betam ='',1pe13.5,
     *      '' beta ='',e13.5/
     *          ''       Reset betam to beta'')') betam, beta
	...
	call set_fdif(x,y,f,xm,xxh,xxhm,xxhp,xxhpm,xxhn,yyh,yyhm,yyhn,
     *    asfdif,bsfdif,asfdfm,bsfdfm,
     *    difx,difxm,alpha,alpham,beta,betam,difsff,difsfm,derdr,
     *    idiags)
	if(idiags.ge.2) write(istdpr,'(a,2i5,1p8e13.5)') 'DGRDER1',
     *    nll,n,x,difsff,xxh,xxhn, (dr-drm)/(x-xm),derdr
        if(derdr.lt.derdr_min.and.-beta.gt.2*derdr_min+0.5) then
c
c  iterate for difsf to obtain derivative derdr_min
c
          it=0
          difsfp=difsff
          derdrp=derdr
          dderp=derdr-derdr_min
          dder_pl= 1.d30
          dder_mn=dderp
          difsf_mn=difsfp
          if(idiags.ge.2) 
     *      write(istdpr,*) 'it, difsfp, derdrp, difsff, derdr',
     *      it,difsfp,derdrp,difsff,derdr
          difsff=difx(1)
c
c  iteration loop
c
   20       it=it+1
            call set_fdif(x,y,f,xm,xxh,xxhm,xxhp,xxhpm,xxhn,yyh,
     *        yyhm,yyhn,asfdif,bsfdif,asfdfm,bsfdfm,
     *        difx,difxm,alpha,alpham,beta,betam,difsff,difsfm,derdr,
     *        idiags)
            if(idiags.ge.2) write(istdpr,'(a,2i5,1p8e13.5)') 'DGRDER1',
     *        nll,n,x,difsff,xxh,xxhn, (dr-drm)/(x-xm),derdr
            dder=derdr-derdr_min
	    if(abs(dder).le.1.and.it.ge.5) then
c
c  left empty on purpose
c
            else if(dder.gt.0) then
              difsfp=difsf_mn
              dderp=dder_mn
            else 
              difsfp=difsf_pl
            ...
            if(dderp.lt.1.d20) then
	      derdif=(difsff-difsfp)/(dder-dderp)
              ddifsf=-dder*(difsff-difsfp)/(dder-dderp)
              if(difsfp.eq.0) then
		ddifsfn=max(-difsff/2.,10*ddifsf)
		if(idiags.ge.2)
     *            write(istdpr,*) 'Reset ddifsf from',ddifsf,'  to',
     *            ddifsfn
		ddifsf=ddifsfn
              end if	  
            else
            ...
            if(idiags.ge.2) 
     *        write(istdpr,'(''it, difsfp, etc.'',i3,1p5e13.5)')
     *        it,difsfp,dderp,difsff,dder,ddifsf
            if(dder.gt.0.and.dder.lt.dder_pl) then
              dder_pl=dder
              difsf_pl=difsff
            else if(dder.lt.0.and.dder.gt.dder_mn) then
              dder_mn=dder
              difsf_mn=difsff
            end if
	    difsfp=difsff
	    dderp=dder
            if(it.le.20.and.abs(dder).ge.1.d-5) then
              difsff=difsff+ddifsf
              go to 20
            end if
c
c  end of iteration loop
c
	  if(abs(dder).gt.1.d-3) then
c
c  lack of convergence. Reset difsf to zero
c
	    write(istdou,
     *        '(/'' ***** Iteration unconverged in s/r dgrder; dder ='',
     *        1pe13.5/
     *           ''       difsf reset to zero.''/)') dder
	    if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,
     *        '(/'' ***** Iteration unconverged in s/r dgrder; dder ='',
     *        1pe13.5/
     *           ''       difsf reset to zero.''/)') dder
	    difsff=0
	  else if(idiags.ge.1) then
            write(istdpr,'('' Convergence after'',i3,'' iterations.'',
     *        '' x, difsf/difx(1) ='',1p2e13.5)') it, x, difsff/difx(1)
          end if
        end if
c
...
c 
c  possibly set derivatives, so far only wrt log f and log T
c
	if(difsff.ne.0) then
	  akf=rkr*rho(2)
          akt=rkt+rkr*rho(3)
          akx=akxa+rkr*rho(4)
          drder0 = dr*amm*((akpp+1)*f(2)+(akpt-4)*f(3)+f(ilum)-1)
	  dbeta2 = amm*(beta*(akf+pt(2))-amm*dr*(-(akpp+1)*f(2)*pt(2)+
     *             (akpt-4)*f(3)*akf))
	  dbeta3 = amm*(beta*(akt+pt(3)-4)-amm*dr*(-(akpp+1)*f(2)*pt(3)+
     *             (akpt-4)*f(3)*(akt-4)))
	  difsf(2)=derdif*dbeta2
	  difsf(3)=derdif*dbeta3
	  if(idiags.gt.0) write(istdpr,
     *      '('' Derivatives, relative derivatives'',1p4e13.5)')
     *      (difsf(i),i=2,3), (difsf(i)/difsf(1),i=2,3)
	end if
c
      end if
c
c  store values for previous meshpoint
c
      do i=1,2
c..	xxhm(i)=xxhn(i)
	xxhm(i)=xxh(i)
	...
	alpham(i)=alpha(i)
      ...
      subroutine set_fdif(x,y,f,xm,xxh,xxhm,xxhp,xxhpm,xxhn,yyh,yyhm,
     *  yyhn,asfdif,bsfdif,
     *  asfdfm,bsfdfm,difx,difxm,alpha,alpham,beta,betam,
     *  difsf,difsfm,derdr,idiags)
c
      implicit double precision (a-h, o-z)
      dimension y(*), f(*), xxh(*), xxhm(*), xxhp(*), xxhpm(*), xxhn(*), 
     *  yyh(*), yyhm(*), yyhn(*), asfdif(*), bsfdif(*),
     *  asfdfm(*), bsfdfm(*), difx(*), difxm(*), alpha(*), alpham(*)
c  
c  Eventually solve to determine difsf such as to obtain zero gradient
c  in radiative gradient (and hence marginal stability). If the resulting
c  difsfm .lt. 0, settling with no fudge gives stable region and
c  difsfm is returned as 0.
c
c  Note: idiags has been set to 0 if istdpr .le. 0, in calling
c  routine
c
c  Original version: 12/9/05.
c
      dimension aa(2,2,2),bb(2,2)
      common/ln10/ amm
      common/step/ dt
      common/cmpstr/ ispxx3, ispcno, icvxx3, icvcno, ibcxx3, ibccno,
     *  isphec, idcomp, iccomp
...
c
      common/cstdio/ istdin, istdou, istdpr, istder
c
c  set equations for new X_i and Y_i
c
      save
c
      dx=x-xm
      al=amm*10.d0**x
      alm=amm*10.d0**xm
      fx=asfdif(1)*y(5+idcomp)/difx(1)+bsfdif(1)*y(4)/difx(1)
      fy=al*(xxh(1)-xxhp(1))/dt
      fy1=(yyh(1)-yyhm(1))/dx
      do i=1,2
	difi=difx(i)+difsf
c..	difim=difxm(i)+difsfm
	difim=difxm(i)+difsf
	aa(1,1,i)=2-dx*bsfdif(i)/difi
	aa(1,2,i)=-dx*asfdif(i)/difi
	aa(2,1,i)=dx*al
	aa(2,2,i)=-2.d0*dt
	bb(1,i)=(2+dx*bsfdfm(i)/difim)*xxhm(i)
     *         +(dx*asfdfm(i)/difim)*yyhm(i)
	bb(2,i)=dx*(al*xxhp(i)-alm*(xxhm(i)-xxhpm(i)))-2.d0*dt*yyhm(i)
c
      end do
      if(idiags.ge.3) then
        write(istdpr,*) '#D# lhs1, rhs1:',
     *    aa(1,1,1)*xxh(1)+aa(1,2,1)*yyh(1),bb(1,1)
        write(istdpr,*) '#D# lhs2, rhs2:',
     *    aa(2,1,1)*xxh(1)+aa(2,2,1)*yyh(1),bb(2,1)
        write(istdpr,'('' aa, bb:''/(1p3e13.5))') 
     *    ((aa(i,j,1),j=1,2),bb(i,1),i=1,2)
c
	write(istdpr,*) 'x, xm, dx, dt:',x, xm, dx, dt
	write(istdpr,*) 'difx(1), difsf, asfdif(1), bsfdif(1):',
     *    difx(1), difsf, asfdif(1), bsfdif(1)
	write(istdpr,*) 'xxhm, xxhp, xxhpm, yyhm:',
     *    xxhm(1), xxhp(1), xxhpm(1), yyhm(1)
      end if
c
c  solve equations
c
      do i=1,2
	det=aa(1,1,i)*aa(2,2,i)-aa(2,1,i)*aa(1,2,i)
	xxhn(i)=(bb(1,i)*aa(2,2,i)-bb(2,i)*aa(1,2,i))/det
	yyhn(i)=(aa(1,1,i)*bb(2,i)-aa(2,1,i)*bb(1,i))/det
      end do
      if(idiags.ge.2) write(istdpr,*) 'xxhn(1), yyhn(1):', 
     *  xxhn(1), yyhn(1)
c
c  set new gradient in radiative gradient
c
      derdrx=0.d0
      do i=1,2
	derdrx=derdrx+(alpha(i)+alpham(i))*(xxhn(i)-xxhm(i))
      end do
      derdr=(derdrx/dx - beta - betam)/2.d0
      if(idiags.ge.2) write(istdpr,'(''DGRDER2'',1p10e13.5)')
     *  derdrx/(2*dx), beta, betam, derdr,
     *  (xxhm(i),i=1,2), (xxhn(i),i=1,2), (yyhn(i),i=1,2)
      return
      end
