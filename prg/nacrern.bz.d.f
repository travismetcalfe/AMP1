...
c
c  Modified 21/10/05, to correct error in CNO rates.
...
c  Note: engenr.n.d.incl replaced by engenr.bz.d.incl, 21/9/05
...
c
        ...
        s(4,4)=3.12e-4
c
c 14N+p scaled down using revised S-value from Angulo et al (2005)
...
c
        return
  105   format('Some coefficientts, s(1,3)',e13.5,' a(4) :',e13.5,
...
c
     ...
     *     write(istdpr,221) k, kpar, a(kpar), b(kpar), zzscr(k), t9
c
c End test
c
...
c
     ...
     *     write(istdpr,*) 'aexx = ',aexx,' coexp = ',coexp
c
c End test
c
...
c Test 
c
            if(istdpr.gt.0.and.idgeng.gt.0) 
     *        write(istdpr,*) 'Null reaction rate'
c
c End test
c
...
c
            if(istdpr.gt.0.and.idgeng.gt.0) 
     *        write(istdpr,*) 'k before skipping O16 reaction : ',k
c
c End test
c
...
c
c  setting of reaction rates including the screening factor
c
...
c            
              if(idgeng.ge.2.and.istdpr.gt.0) 
     *          write(istdpr,*) 'reaction O16(p,g)F17 reaction rate =', 
     &                al(1,k)
c
c End test
c
...
c
              if(idgeng.ge.2.and.istdpr.gt.0) then 
                write(istdpr,*) 'Reaction rates in cnrnacre'
                do j=1,11
                  write(istdpr,*) al(1,j)
                enddo
c
