...
c
  ...
  150 format(/' In s/r rczmsh ifxcon has been set to 1,',
     *  ' nmxcor = nff(inc) to',i5)
      ...
      subroutine rczms1(x,y,yp,nn,iy,ncz,xcz,npts,icase,istrt,idiag)
c
...
c
      save
c
      if(istdpr.gt.0) write(istdpr,'(/'' Entering s/r rczms1''/)')
      n1=ncz-npts
      n2=ncz+npts
      dx1=(xcz-x(n1))/npts
      dx2=(x(n2)-xcz)/npts
...
c
	...
	do 60 n=1,npts21
	ns=n1+n-1
   60   write(istdpr,135) ns,x(ns),xnew(n),10.d0**x(ns),10.d0**xnew(n),
     *    y(ist2(2),ns),ynew(ist2(2),n)
      end if
c
      kst=kst1+kst2
      do 62 n=1,npts21
...
c
  ...
  130 format(//' Reset quantities in s/r rczmsh.'/
     *  ' n_orig, x_orig, x_new, q_orig, q_new, Xh_orig, Xh_new:'/)
  135 format(i5,1p4e15.7,0p2f10.7)
      end
