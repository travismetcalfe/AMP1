...
c  all cases.
c
c  Modified 19/11/03, including also derivative of opacity wrt Z
c  in dac, as ddac(6)
c  (Note that dak(4) = d log kappa/d Z is assumed to have been
c  set in the calling subroutine)
c
...
c
      save
c
      data etacp,phcp /-1.,-1./,is/2,3,5,1,4,6/
c
      idgmxl=0
c
...
c
      ddr=dr*(amm*ddr+dtgrfc(j)/tgrfct)
      dlrr=amm*dlrr
c
      da=-a*(dlrr*ddrad+ddr-dadi)/2
      ddac(j)=dadi+ddy*da+y*(y+a)*(ddr-dadi)
c
c  Note: diagnostic storage should be extended, but that requires care
c  with common/cnvout/
c
      if(j.le.5) then
        adrr(j)=rr*dlrr
        addr(j)=ddr
