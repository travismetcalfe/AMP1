...
c
      parameter(naztmx = nspcmx+3, iy = 2*(ivarmx+nspcmx), 
...
c
     ...
     *  lastmd, concor, nscfil
     ...
     .  thetad(ivarmx),thetdf(ivarmx),dwf(40,1),
...
c
     ...
     *    idiffct, itbdif, tlfdif,dtldif, ctdfmx, rctdif,rctdf1
        ...
        write(istdou,*)  
     *    idiffct, itbdif, tlfdif,dtldif, ctdfmx, rctdif,rctdf1
        write(istdou,*) 'ismdif, nsmdif?'
        write(istdou,*)  ismdif, nsmdif
        read(istdin,*,end=90) ismdif, nsmdif
        write(istdou,*) '     -----------  New values  --------'
        write(istdou,*)  ismdif, nsmdif
        write(istdou,*) 'thetdf?'
        write(istdou,*)  thetdf
...
c
      if(ipartr.eq.1.and.(itriab.eq.1.or.idatmd.eq.0)) then
        write(istdou,190) 
        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,190)
        go to 20
      end if
c
...
c
        if(modeeq.eq.0) then
          ...
          if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,1002) modeeq
...
c  or printing for given model. Also possibly reset nxhzer
c  (setting of nxhzer is a mess that deserves clearing up)
c
...
c
     ...
     *      idiffct, itbdif, tlfdif,dtldif, ctdfmx, rctdif, rctdf1, tail
        write(istdpr,*) 'ismdif, nsmdif'
        write(istdpr,*)  ismdif, nsmdif, tail
        write(istdpr,*) 'thetdf'
...
c
	...
	inmixc=0
        it=-1
c..	write(6,*) 'Just before predictor call of cmpcvc'
...
c
      if(idgn77.ne.0) then
        ...
        write(77) nn,nvar,dt,idcomp
      end if
c
      if(idgc77.ne.0.and.ntime.eq.1) then
...
c
     ...
     *    initcp)
     ...
     .    ii1,ii2,ii3,kk,ka,kb,ki,nn,iy,ucy,ea,det,v,thetdf,dt,iter)
c
...
c
      if(istdpr.gt.0) then
        if(itndgn.ne.1.and.istdpr.gt.0) then
          write(istdpr,112) it,(ea(k,1),k=1,ii),eam
        else
c
          write(istdpr,111) it,eam
          do 58 j=1,3
...
c
	...
	go to 60010
      else if(it.gt.iterlm.and.eam.gt.2*eamp) then
	itwarn=itwarn+1
...
c
     ...
     *      '' iform,nn,nrdtmd,nidtmd,nvar,nbcprv ='',6i5)')
     *      ntime,iform,nn,nrdtmd,nidtmd,nvar,nbcprv
...
c
      if(ntime.lt.0) then
        ...
        if(istdou.ne.istdpr.and.istdpr.gt.0) write(istdpr,*) 
     *    ' Error. ntime .lt.0'
...
c
      dls=log(alsfin/alls)
      drs=log(rsfin/arrs)
      write(istdou,170) itlr,alfa,sbcfct,xhinit,alls,arrs,dls,drs
      if(istdpr.ne.istdou.and.istdpr.gt.0) 
     *  write(istdpr,170) itlr,alfa,sbcfct,xhinit,alls,arrs,dls,drs
c
c  test for convergence
c
...
c  test for excessive number of iterations
        if(itlr.ge.nitlr) then
          write(istdou,174) nitlr
          if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,174) nitlr
        else
...
c
c  test for applying undercorrection factor
c
          if(itlr.ge.6) then
	    ucy_lr=0.7
	    if(istdpr.gt.0) write(istdpr,
     *        '(/'' Apply undercorrection factor'',f6.3/)') ucy_lr
	    dalfa=dalfa*ucy_lr
	    dxxh=drxxh*ucy_lr
          end if
c
...
c
          ...
          if(iterlr.le.2.or.iterlr.ge.5) then
            write(istdou,175) dalfa,dxxh,alfa,xhinit
            if(istdpr.ne.istdou.and.istdpr.gt.0) 
     *        write(istdpr,175) dalfa,dxxh,alfa,xhinit
          else
            write(istdou,176) dalfa,dxxh,sbcfct,xhinit
            if(istdpr.ne.istdou.and.istdpr.gt.0) write(istdpr,176) 
     *        dalfa,dxxh,sbcfct,xhinit
          end if
...
c
 ...
 1002 format(//' **** error in evolution programme.'/
  ...
  174 format(//1x,10('*'),' iteration failed to converge in less than',
     *  i4,' iterations')
