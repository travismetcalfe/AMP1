     ...
     *  aldtrt,dtfcrd,dtceps,iy,nn,initdt)
c
...
c
c  Modified 13/12/04, adding argument initdt such that initdt = 1
c  allows large increase in timestep (overriding internal flag init)
c
c  Modified 17/7/05, increasing allowed increase in timestep for
c  very small change in model.
c
...
c
c  set test for convective core (possibly with semiconvection)
c
      if(nmxscn.gt.0) then
        ncc=max0(nmxscn,nmxscp)+1
      else if(nmxcor.gt.0) then
        ncc=nmxcor+1
      else
        ncc=nn+1
      end if
...
c
      ...
      qmxcor_max=max(qmxcor,qmxcp)
      if(qmxscp.gt.0) then
	qmxscn_min=min(qmxscn,qmxscp)
      else
        qmxscn_min=qmxscn
      end if
c
...
c  
c  Also, neglect semiconvective region, if ixcscn = 1
c  (so far hard-coded above)
c  
      if(i.eq.2.or.i.eq.5.or.(iheccs.ne.0.and.i.eq.iyche4)) then
        if((n.le.nmxcp.and.n.ge.nmxcor-1).or.
     *    (ixcenv.eq.1
     *       .and.xrclp(1)+0.01.ge.x(n).and.x(n).ge.xrcl(1)-0.01).or.
     *    (ixcscn.eq.1.and.imixc5.gt.0.and.nmxscn.gt.0.
     *       .and.qmxscn_min.le.q.and.q.le.qmxcor_max))
     *       chmxn=0
      end if
c  
...
c
      if(istdpr.gt.0) write(istdpr,'(/a/(i4,i5,1p2e13.5))')
     *  ' Changes in s/r setdt. i, nmax, q(nmax), max. change:',
     *  (i,nymax(i),10.d0**x(nymax(i)),dymax(i),i=1,4+nspec)
c
      dtp=dt
      dtn=dt*dymx/chmx 
c
...
c  For very small dymx allow a somewhat larger increase.
c  Note: the value of the limit may still have to be adjusted.
c
      if(init.eq.0.and.initdt.eq.0) then
        if(chmx.le.0.1*dymx) then
          dtn=min(dtn,2.0*dt)
        else
          dtn=min(dtn,1.4*dt)
        end if
...
c  changes to file
c
      if(idgsdt.lt.0.or.(idgsdt.lt.1.and.dt/dtp.ge.0.5)) return
c
      do 60 n=1,nn 
      do 55 i=1,nvar
      yy=y(i,n)
