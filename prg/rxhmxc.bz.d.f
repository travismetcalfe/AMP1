      subroutine rxhmxc(x,y,iy,nn,compc,qc,rxmn,drxmn,icomp,alres)
c
...
c
     ...
     *  write(istdpr,*) 'in rxhmxc, xhc, zh =', xhc, zhc
      if(irxprt.gt.0.and.istdpr.gt.0) write(istdpr,*) 'n, q, fl, T, rho'
      do 30 n=nc1,nn
      qq=10.d0**x(n)
...
c
      if(irxprt.gt.1.and.istdpr.gt.0) write(istdpr,'(a/(i5,1p9e11.3))')
...
c
      ...
      kl=2*icomp2
      amms=amsun*am
      if(idiag.ge.1.and.istdpr.gt.0) write(istdpr,150)
      do 60 n=nnc-1,2,-1
      n1=n1-1
      q1=10.d0**x(n1)
...
c
c..      write(39,'(1p3e13.5)') (x(n),y(4,n),alres(n),n=1,nn)
c
      return
  ...
  150 format(//' Reset luminosity in s/r rxhmxc.'/
     *         ' n, q(mod), q(s/r), org. log L*, new log L*:'/)
  155 format(i5,1p4e13.5)
  157 format(i5,1pe13.5,13x,2e13.5)
