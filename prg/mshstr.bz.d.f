...
c
      ...
      ax1(4)=am2
c
c  test for excluding luminosity contribution
c
...
c
	...
	dxcvc=x(n)-qlmxcr
        ...
        trmcvc=((qmxcor+qmxlim)/(qmxcor+5.d-3))*wmshcc*exp(-dxcvc*dxcvc)
...
c
     ...
     *    aks(naksmx)
        if(iprtlc.eq.0) then 
	  if(istdpr.gt.0) write(istdpr,165) (aks(n),n=1,nn1)
	  iprtlc=1
	end if
        krpeat=krpeat+1
        if(krpeat.ge.3) then
...
c
c..      if(ntime.ge.220)
c..     *  write(istdpr,'(/a/(i5,1p5e15.8))') 
c..     *  ' n, xsi, xsi, X1, Y1, X2, Y2',
c..     *  (n, aks(n), yw(5,n),yw(iyche4,n),yw(5+nvar,n),yw(iyche4+nvar,n),
c..     *   n=500,600)
c
      j_daks=0
      do 70 n=2,nn
...
c
	  ...
	  if(n.eq.1) then
	    frcf(icf)=1
            ...
            if(istdpr.gt.0) then
	      write(istdpr,*) 'Setting frcf at n =',n, ' for icf =',icf
...
c
c  reset composition to central value
...
c
      ...
      character*(*) label
      logical ldisc
      dimension x(1), y(iy,1), dydx(idy,1), xc(1), nc(1)
...
c
      dimension x(1), y(iy, 1), yw(iywst,1)
...
c
      ...
      parameter(iyint = 2*ivarmx+1)
c
      dimension x(1), y(iy, 1), yw(iywst,1),qll(1)
      dimension yint(iyint)
      common/noiter/ iter, ntime
      common/sooner/ xw(1)
      common/convpt/ dcs,dcc,nf(6),nl(6),inc,jnc,frcf(6),frcl(6)
...
c
      include 'engenr.bz.d.incl'
      dimension x(1),al(ial,1),dl(idl,1)
...
c
      implicit double precision(a-h, o-z)
      dimension ncdisc(1), xcdisc(1), npdisc(1), xpdisc(1)
