...
c
	...
	datgng(56)=zh(1)
c
        write(idssum,140) agey,rs,te,als,dcz,(aztus(i),i=1,4),rhoc,epsc,
     .    rcnoc,uwc,r1,amc,alc,cvr(2,nn+1)
	if(icnocs.ge.1) then
...
c
     ...
     *      dradc, dadc,qmxcor,rmxcor,cvr(2,nn+1),y(5,1),zh(1)
          ...
          call extcno(xcno,nn+1)
          write(idscen,150) amass, (datgng(i),i=1,12), 
     *      dradc, dadc,qmxcor,rmxcor,cvr(2,nn+1),(xcno(k),k=1,icvcno),
     *      y(5,1),zh(1)
          if(iducen.ge.0) write(iducen) amass, (datgng(i),i=1,12), 
     *      dradc, dadc,qmxcor,rmxcor,cvr(2,nn+1),(xcno(k),k=1,icvcno),
     *      y(5,1),zh(1)
...
c
     ...
     *    n=1,nnosc)
c
        if(istdpr.gt.0) write(istdpr,170) 
     *    nmodos,data(5),data(2),data(3),data(4)
      end if
c
c  set new variables
c
...
c
      common/cdsdsg/ idstrl,idshvz,idszab,idsevl,idssum,idscen,idsnov,
     *  idsoov,idsgn1,idsgsm,idstm1,idstm2,iducen
c
c  common defining standard input and output
c
      common/cstdio/ istdin, istdou, istdpr, istder
...
c
        if(.false.) write(istdpr,*) 'Start setting diff. quantities',
     *    ' csdifr(1,300) = ',csdifr(1,300)
c
	do 33 n=1,nn
        if(n.lt.natm) then
	  do 31 i=45,46+2*idcomp
   31     yvar(i,n)=0
	else
c
