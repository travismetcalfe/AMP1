...
c     get data: these are initial parameters + data + errors
...
c     set up the initial parameters 5 times for different Z 
      do i = 1, 5
         do j = 1,7     
            dat(j) = spar(j)
         enddo       
      ...
      enddo
c     set up initial parameters again using the Z value with lowest Chi^2
      do i = 1, 4
         do j=1,7 
            dat(j) = spar(j)
         enddo
...
c
      ...
      do i = 1, 5
         ...
         print *,dat(1),dat(2),dat(3),dat(4),dat(5),'input is X'
         call reducemrq(dat,yi,sig,ia,subscripts, npar,nobs,chisq,retx)
         if (chisq.lt.ochisq) then
            szz = zdiff1(i)*5.
            ochisq = chisq
...
c     go to z again if we are still getting better answers
   ...
   10 continue 
      ...
      print *,dpar(1), 1.0 - dpar(2)-dpar(3), dpar(3), dpar(4), dpar(5)
