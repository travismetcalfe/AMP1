...
c  check order  
      k2=2*k
      if(nt-k2) 10,15,15
   10  write(istdou,100) k2,nt 
       if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,100) k2,nt 
      k=nt/2
      k2=2*k
...
c  we do not allow extrapolation
   90  write(istdou,110) x,xi(1),xi(nt)
       if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,110) 
     *   x,xi(1),xi(nt)
      inter=0   
      return
   95 write(istdou,120) xi(1)
      write(istdou,125) (i,(xi(i+j),j=0,min(4,nt-i)),i=1,nt,5)
      if(istdpr.gt.0.and.istdpr.ne.istdou) then
	write(istdpr,120) xi(1)
        write(istdpr,125) (i,(xi(i+j),j=0,min(4,nt-i)),i=1,nt,5)
      end if
      inter=-1  
      return
   97 i1=i+1
      ik2=i+k2  
      write(istdou,130) xi(1),xi(nt),i1,xi(i1),ik2,xi(ik2)   
      if(istdpr.gt.0.and.istdpr.ne.istdou) write(istdpr,130) 
     *  xi(1),xi(nt),i1,xi(i1),ik2,xi(ik2)   
      inter=-1  
      return
   98 i1=i+1
      ik2=i+k2  
      write(istdou,140) (l,xi(l),l=i1,ik2)   
